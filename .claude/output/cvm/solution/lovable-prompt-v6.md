# Lovable Prompt — CVM System v6
> Cập nhật: Campaign Builder — trigger conflict only for OR, campaign-level trigger priority, Section 5 Safety & Anti-spam with whitelist, Section 6 trigger-based sending configuration, remove Review & Submit section
...
Goal: support both simple campaigns with 1 trigger and advanced campaigns with multiple triggers. The UI must reduce confusion by separating campaign info, trigger logic, audience, message matrix, safety/anti-spam rules, and trigger-based delivery configuration.
...
- When Basic mode is clicked:
  - Show only one field: "Trigger chính *" as searchable dropdown.
  - Render one selected trigger card.
  - Hide OR/AND controls completely.
  - Show note: "Basic mode chỉ dùng 1 trigger nên không cần AND/OR. Message Matrix sẽ hiển thị 1 message theo từng kênh."
- When Advanced logic switches to AND:
  - Keep all selected trigger cards visible.
  - Switch explanation to: "Chỉ gửi khi KH đồng thời thỏa tất cả trigger."
  - Show checklist: đã kích hoạt SIM, sau 24h vẫn chưa cài app, data còn lại < 100MB.
  - Message Matrix must switch to one shared message per channel.
- Add campaign-level trigger conflict handling in Advanced mode:
  - Radio option 1 selected by default: "Chỉ gửi trigger ưu tiên cao nhất"
  - Radio option 2: "Gửi tất cả trigger đã match (không khuyến nghị)"
  - Show draggable priority list:
    - P1 T3 LOW_DATA_BALANCE
    - P2 T2 NO_APP_INSTALL_24H
    - P3 T1 SIM_ACTIVATED
  - Priority only applies inside this campaign and does not change trigger master priority.
- Add collapsible block "Lịch xử lý trigger" inside Section 2:
  - Collapsed summary: "T1 Realtime · T2 Near realtime 24h · T3 Batch 09:00"
  - Expanded view shows schedule/blackout handling per trigger.
  - Do not create a separate long Section 6 for trigger schedule.
...
- Searchable dropdown input: "Tìm và chọn phân khúc..."

Segment dropdown behavior:
- Do not show the full segment list permanently on the page.
- Only show the segment list inside the dropdown/popover when the user clicks/searches.
- Dropdown rows include checkbox, segment name, and customer count:
...
- When a row is checked, add it to selected badges.
- When unchecked or badge × is clicked, remove it and recalculate reach.
...
- When "+ Thêm điều kiện" is clicked, append a new condition row:
  [Attribute dropdown] [Operator dropdown] [Value input/dropdown] [×]
- Attribute groups:
...
- Image controls: [Upload] [Chọn từ thư viện]
- Title: "Chào mừng bạn!"
- Body: "SIM eSIM đã kích hoạt thành công. Tải MyViettel..."
- Preview area: Push notification preview with app icon, title, body, and optional 1:1 thumbnail
...
- SMS/USSD do not show image upload controls.
- SMS shows counter X/160 and warning state if content exceeds limit.
- SMS can exceed 160 characters; do not block saving.
- Billing/cost calculation for SMS uses `ceil(character_count / 160)`.
- Example: 300/160 ký tự = 2 SMS segments. Cost estimate must multiply by 2 for that message.

When switching to Zalo OA tab:
- Render the same trigger rows, but each row uses Zalo-specific fields:
  - Template dropdown, e.g. "Welcome Zalo OA"
  - Rich/plain message body with variables such as {{ten_kh}}, {{data_con_lai}}
  - Optional image controls: [Upload] [Chọn từ thư viện]
  - Optional CTA/link if the selected template supports it
  - Preview: Zalo brand header + chat bubble + optional image
- Sample rows:
  - T1 Zalo Welcome: ✓ configured
  - T2 Zalo Chưa cài app: ✓ configured
  - T3 Zalo Sắp hết data: ✓ configured

When switching to Banner tab:
- Render trigger rows with Banner-specific fields:
  - Image upload required, ratio 16:9 recommended
  - Title input
  - Body textarea
  - CTA button label
  - Deep link / destination URL
  - Preview: 16:9 banner card with title, body, CTA
- Sample rows:
  - T1 Banner Welcome: ✓ configured
  - T2 Banner Chưa cài app: ⚠ Chưa cấu hình
  - T3 Banner Sắp hết data: ⚠ Chưa cấu hình

When switching to Email tab:
- Render trigger rows with Email-specific fields:
  - Subject input
  - Optional header image upload
  - Rich text body editor
  - CTA label and link
  - Preview: email client style with subject, header image, and body
- Sample row:
  - T1 Email Welcome: ⚠ Chưa cấu hình

When switching to USSD tab:
- Render trigger rows with USSD-specific fields:
  - Plain text/menu textarea only
  - Character counter, e.g. 0/182 ký tự
  - No image upload controls
  - Warning text: "USSD không hỗ trợ ảnh; cần kiểm tra tiếng Việt có dấu"
  - Preview: terminal/menu-style USSD screen
- Sample row:
  - T1 USSD Welcome: ⚠ Chưa cấu hình

When Advanced logic = AND:
- Replace trigger-specific message cards with one shared message card per channel.
- Show one shared message card for the active channel, still using that channel's own fields and preview format.
- Example Push shared message:
  - Header: "Message chung · Push"
  - Condition label: "T1 + T2 + T3 cùng thỏa"
  - Audience variant: [Gen Z User]
...
When "+ Thêm biến thể đối tượng" is clicked:
- Open modal/drawer titled "Thêm biến thể đối tượng cho [Trigger code]".
- Fields:
  - Segment dropdown: e.g. "Khách hàng lớn tuổi"
  - Channel label: current active channel
  - Template dropdown
  - Channel-specific content fields
  - Preview
- Buttons: [Hủy] [Thêm biến thể]
- After adding, the trigger card shows multiple audience variants, e.g.:
  - [Gen Z User] → Low Data Gen Z Push
  - [Khách hàng lớn tuổi] → Low Data Basic Push

Preview by channel:
- Push: app icon, title, body, optional 1:1 thumbnail.
- Zalo OA: brand header, bubble content, optional image.
- SMS: plain text, no image, X/160 counter.
- Banner: 16:9 image, title, body, CTA.
- Email: subject, header image, rich text body.
- USSD: terminal/menu preview, plain text only, no image, character counter and unsupported-character warning.

Channel-specific required fields:
- Push: template or title + body. Optional image 1:1 and deep link.
- Zalo OA: template or message body. Optional image and CTA/link when supported.
- SMS: SMS body only. No image.
- Banner: image 16:9, title, body, CTA.
- Email: subject and body. Optional header image and CTA.
- USSD: plain USSD text/menu only. No image.

When clicking "+ Thêm kênh":
- Open modal/dropdown "Thêm kênh gửi".
- Show only unselected channels as enabled options.
- Selected-existing channels must be disabled.
- Options include Push App, Zalo OA, SMS, Banner, Email, USSD.
- After adding a channel, create a new tab and add that channel to the completion summary.
...
Build this section like an operational safety card similar to the attached reference.

Blackout block:
- Label: "Giờ giới nghiêm (Blackout)"
- Time inputs: [22:00] - [08:00] with clock icons
- Dropdown: "Hủy luôn (Discard)" selected by default
- Dropdown options: Queue đến giờ cho phép / Hủy luôn (Discard) / Gửi ngay nếu kênh cho phép
...
DNC block:
- Checked checkbox: "Check blacklist DNC"
- Green status text: "Enabled"

Whitelist block:
- Radio options:
  - "Không dùng whitelist"
  - "Chỉ gửi cho tệp whitelist được chọn" selected
- Whitelist selector:
  - Selected value: "WL_ESIM_Q2_2026 · 1,250 KH · cập nhật 12/05/2026 08:00"
  - Buttons: [Xem danh sách] [Thay tệp] [Upload tệp mới]
- Reach result text: "Sau khi áp whitelist: reach còn ~1,120 KH"
- Explain visually that whitelist narrows the selected Audience. Customers outside the whitelist must not receive messages.
...
- Preview area:
  ...
  - Show sample phone rows masked
- Footer buttons: [Hủy] [Áp dụng whitelist]

Behavior:
- Blackout, frequency cap, DNC, and whitelist are campaign-level safety rules.
...
- Updating whitelist must recalculate reach and sticky summary immediately.
...
Build this as a full section, not an accordion inside Section 2.

Info banner:
- Text: "Thời điểm gửi phụ thuộc vào trigger; không có lịch gửi chung cho toàn campaign."

Trigger schedule cards:
- T1 · SIM_ACTIVATED
  - Kiểu chạy: Realtime
...
Do not render a separate Review & Submit section. Approval actions stay in the fixed header only.
...
- Clicking `Banner: 1/3` scrolls to the missing Banner message.
- Cost/SMS risk updates when changing segment, whitelist, frequency cap, trigger send config, or message completion.
