...
Each selected trigger renders as an expandable card (not just a tag):
- Header row: [★ or blank] [Trigger name] [✕ remove]
  - ★ star icon = trigger chính (primary trigger)
- Expanded body shows payload list:
  - SIM_ACTIVATE payload: ten_kh (BSS), loai_sim (BSS), ngay_kich_hoat (BSS), so_dien_thoai (BSS)
  - TOP_UP_FAIL payload: so_lan_nap_sai (BSS), so_tien_nap (BSS), ten_kh (BSS)
  - DATA_EMPTY payload: data_con_lai (OCS), ten_goi (OCS), ngay_het_han (OCS)
- Payload items show source badge: [BSS] or [OCS]

Button: "+ Thêm trigger"
- Click → dropdown with search + list:
...
- Click item → add as expandable card

"Trigger chính" selector (shown only when 2+ triggers selected):
- Label: "Trigger chính:"
- Dropdown listing all selected triggers
- Default: first trigger added
- Info tooltip: "Tham số trong tin nhắn sẽ lấy từ trigger này"
- When only 1 trigger: hide this selector, that trigger is automatically primary

Behavior:
- Changing primary trigger → parameter chips in middle column update instantly
- Removing primary trigger → auto-select next trigger as primary
- ★ badge on trigger card updates to reflect current primary
...
- Parameter chips section:
  - Label: "Tham số (tên_trigger_chính):" — e.g. "Tham số (SIM_ACTIVATE):"
  - Chips show ONLY the payload of the primary trigger
  - Sample chips for SIM_ACTIVATE: [{{ten_kh}}] [{{loai_sim}}] [{{ngay_kich_hoat}}] [{{so_dien_thoai}}]
  - Click chip → insert at cursor position in textarea
  - If textarea contains a param NOT in primary trigger's payload → highlight that param in red + tooltip: "Tham số này không thuộc trigger chính hiện tại"
- SMS channel only: character counter "X/160 ký tự" — turns red when > 160

Behavior when primary trigger changes:
- Parameter chips update immediately to new primary trigger's payload
- Existing textarea content is preserved
- Any params no longer valid → highlighted red with tooltip warning
...
- Sample params section:
  - Label: "Tham số mẫu (tên_trigger_chính):" — e.g. "Tham số mẫu (SIM_ACTIVATE):"
  - Shows ONLY params from primary trigger's payload as editable inputs:
    - ten_kh = [Nguyễn Văn A]
    - loai_sim = [eSIM]
    - ngay_kich_hoat = [01/04/2026]
    - so_dien_thoai = [0987 xxx 001]
  - Editing any input → preview re-renders immediately
- SMS warning: "⚠ 142/160 ký tự" shown below preview when SMS selected
- When primary trigger changes → sample param fields update to new trigger's payload
...
- Dynamic parameter chips below textarea:
  [{{ten_kh}}] [{{so_du}}] [{{data_con_lai}}] [{{ten_goi}}] [{{ngay_het_han}}] [{{loai_sim}}] [{{ngay_kich_hoat}}]
...
- Payload fields section:
  ...
  - List of payload params (add/remove rows):
    - [Tên tham số *] [Nguồn: BSS / OCS ▾] [✕]
  - Button: "+ Thêm tham số"
...
- Trigger cards in left column: expandable, show payload params with BSS/OCS source badges
- Primary trigger marked with ★ star badge
- Parameter chips in middle column: only show primary trigger's payload, highlighted red if invalid
- Preview panel: sample params sync with primary trigger's payload, updates realtime
