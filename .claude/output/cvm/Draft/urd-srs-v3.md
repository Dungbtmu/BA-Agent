...
| 11/05/2026 | Codex | II.5. Sơ đồ trình tự | M | Chi tiết hóa diễn giải theo từng giai đoạn và từng bước xử lý | V1.2 |
...
**Diễn giải chi tiết theo từng giai đoạn và từng bước:**

**Giai đoạn 1: Truy cập và khởi tạo**

| Bước | Tác nhân | Hành động / Xử lý | Input | Output | Quy tắc / Ghi chú |
|---|---|---|---|---|---|
| 1.1 | QTV | Đăng nhập vào hệ thống CVM bằng tài khoản được cấp | Username, password hoặc phương thức xác thực nội bộ | Yêu cầu đăng nhập được gửi đến CVM | QTV phải là user còn hiệu lực và chưa bị khóa |
| 1.2 | Hệ thống CVM | Xác thực tài khoản và kiểm tra quyền truy cập | Thông tin đăng nhập của QTV | Kết quả xác thực thành công hoặc thất bại | Nếu xác thực thất bại, hệ thống không cho vào Dashboard và hiển thị lỗi đăng nhập |
| 1.3 | Hệ thống CVM | Hiển thị Dashboard sau khi đăng nhập thành công | Kết quả xác thực hợp lệ | Dashboard tổng quan campaign, trigger, số liệu gửi tin | Menu hiển thị theo quyền của QTV; chức năng Admin-only không hiển thị hoặc bị disable |
| 1.4 | QTV | Truy cập phân hệ Campaign và nhấn "+ Tạo Campaign" | Menu Campaign, quyền tạo campaign | Yêu cầu mở màn hình tạo campaign | Chỉ QTV có quyền `CREATE Campaign` mới được tạo mới |
| 1.5 | Hệ thống CVM | Khởi tạo Campaign Builder | Yêu cầu tạo campaign mới | Màn hình Campaign Builder 2 cột ở trạng thái Draft chưa lưu | Hệ thống chưa tạo bản ghi campaign chính thức cho đến khi QTV nhấn Lưu Nháp hoặc Gửi Duyệt |

**Giai đoạn 2: Nhập thông tin và cấu hình**

| Bước | Tác nhân | Hành động / Xử lý | Input | Output | Quy tắc / Ghi chú |
|---|---|---|---|---|---|
| 2.1 | QTV | Nhập thông tin cơ bản của campaign | Tên campaign, mã kịch bản, thời gian hiệu lực | Dữ liệu thông tin chung được ghi nhận tạm trên form | Tên campaign bắt buộc; thời gian kết thúc phải lớn hơn thời gian bắt đầu |
| 2.2 | QTV | Chọn trigger kích hoạt từ danh sách trigger có sẵn | Danh sách trigger Active | Trigger được gắn vào campaign | Phải chọn tối thiểu 1 trigger; nếu chọn nhiều trigger phải có logic kết hợp AND/OR |
| 2.3 | Hệ thống CVM | Load payload của trigger từ cơ sở dữ liệu | Trigger ID đã chọn | Danh sách tham số động BSS/OCS của trigger | Chỉ hiển thị tham số thuộc trigger đã chọn; không cho chèn tham số ngoài payload |
| 2.4 | Hệ thống CVM | Hiển thị payload inline trên Campaign Builder | Danh sách payload từ DB | Các chip tham số động để QTV tham khảo/chèn vào nội dung | Nếu nhiều trigger, payload dùng để render lấy từ trigger chính |
| 2.5 | QTV | Soạn nội dung theo từng kênh gửi và chèn tham số động | Kênh gửi, template hoặc nội dung nhập tay, tham số động | Nội dung nháp cho từng kênh | Mỗi kênh đã thêm phải có nội dung; tham số động phải đúng cú pháp `{{ten_tham_so}}` |
| 2.6 | Hệ thống CVM | Cập nhật preview nội dung realtime | Nội dung QTV đang soạn, tham số động đã chọn | Bản preview hiển thị nội dung dự kiến gửi cho khách hàng | Preview phải phản ánh đúng kênh đang active; tham số chưa có fallback cần được cảnh báo |
| 2.7 | QTV | Cấu hình segment và control group | Điều kiện lọc khách hàng, tỉ lệ control group | Segment rule của campaign | Control group nếu có phải nằm trong khoảng 0-100% |
| 2.8 | Hệ thống CVM | Tính ước tính số khách hàng đủ điều kiện | Segment rule, dữ liệu profile khách hàng có thể truy vấn | Số lượng khách hàng ước tính | Số liệu chỉ mang tính ước tính tại thời điểm cấu hình, không thay thế kết quả lọc lúc gửi thật |
| 2.9 | QTV | Cấu hình gửi và an toàn | Loại gửi, cooldown, daily cap, blackout window, fallback channel | Send policy của campaign | Cooldown tối thiểu 1 phút; khung giờ gửi không được giao với blackout window |

**Giai đoạn 3: Lưu và gửi duyệt**

| Bước | Tác nhân | Hành động / Xử lý | Input | Output | Quy tắc / Ghi chú |
|---|---|---|---|---|---|
| 3.1 | QTV | Nhấn "Gửi Duyệt →" | Toàn bộ dữ liệu campaign trên form | Yêu cầu validate và gửi duyệt | QTV có thể chọn Lưu Nháp trước nếu chưa muốn gửi duyệt |
| 3.2 | Hệ thống CVM | Validate dữ liệu đầu vào | Thông tin chung, trigger, nội dung kênh, segment, send policy | Danh sách lỗi hoặc kết quả hợp lệ | Validate bắt buộc chạy trước khi lưu trạng thái Pending |
| 3.3a | Hệ thống CVM | Xử lý trường hợp dữ liệu không hợp lệ | Danh sách lỗi validate | Field lỗi được highlight, thông báo lỗi hiển thị tại section liên quan | Không tạo/cập nhật campaign sang Pending khi còn lỗi blocking |
| 3.3b | QTV | Chỉnh sửa dữ liệu theo lỗi được hệ thống chỉ ra | Thông báo lỗi validate | Form được cập nhật lại | Sau khi sửa, QTV có thể gửi duyệt lại |
| 3.4 | Hệ thống CVM | Lưu campaign hợp lệ vào cơ sở dữ liệu | Campaign data đã pass validate | Bản ghi campaign trạng thái Pending | Hệ thống cần lưu snapshot nội dung/template tại thời điểm gửi duyệt |
| 3.5 | Cơ sở dữ liệu | Xác nhận lưu thành công | Request lưu campaign | Kết quả lưu thành công | Nếu lưu thất bại, hệ thống phải giữ nguyên trạng thái trước đó |
| 3.6 | Hệ thống CVM | Thông báo gửi duyệt thành công cho QTV | Kết quả lưu Pending thành công | Toast/notification "Đã gửi duyệt thành công" | Campaign xuất hiện trong danh sách Pending cho Admin |

**Giai đoạn 4: Admin duyệt**

| Bước | Tác nhân | Hành động / Xử lý | Input | Output | Quy tắc / Ghi chú |
|---|---|---|---|---|---|
| 4.1 | Admin | Mở danh sách campaign Pending và xem chi tiết campaign | Campaign trạng thái Pending | Màn hình chi tiết campaign chờ duyệt | Chỉ Admin có quyền duyệt mới nhìn thấy action Duyệt/Từ chối |
| 4.2 | Admin | Kiểm tra nội dung, trigger, segment, kênh gửi và cấu hình an toàn | Chi tiết campaign | Quyết định duyệt hoặc từ chối | Nếu từ chối, Admin phải nhập lý do |
| 4.3 | Admin | Nhấn Duyệt | Campaign Pending hợp lệ | Yêu cầu kích hoạt campaign | Không cho duyệt campaign đã hết thời gian hiệu lực |
| 4.4 | Hệ thống CVM | Cập nhật trạng thái campaign thành Active | Campaign ID, quyết định duyệt | Campaign trạng thái Active | Trạng thái Active cho phép Campaign Engine match trigger khi có event phù hợp |
| 4.5 | Cơ sở dữ liệu | Xác nhận cập nhật thành công | Request cập nhật trạng thái | Kết quả cập nhật thành công | Cần ghi audit log: người duyệt, thời điểm duyệt, trạng thái trước/sau |
| 4.6 | Hệ thống CVM | Thông báo campaign đã được duyệt cho QTV | Kết quả cập nhật Active | Notification cho QTV | QTV có thể theo dõi campaign trong danh sách Active và báo cáo |
...
**Giai đoạn 1: Nhận và match trigger**

| Bước | Tác nhân | Hành động / Xử lý | Input | Output | Quy tắc / Ghi chú |
|---|---|---|---|---|---|
| 1.1 | BSS/OCS | Push trigger event sang CVM | Unique Event ID, trigger code, customer identifier, payload nghiệp vụ | Event được CVM tiếp nhận | Unique Event ID là bắt buộc để chống xử lý trùng |
| 1.2 | Hệ thống CVM | Kiểm tra Unique Event ID | Unique Event ID từ event | Kết quả event mới hoặc event trùng | Nếu event trùng, CVM không được gửi tin lại |
| 1.3a | Hệ thống CVM | Xử lý event trùng | Event ID đã tồn tại | Ghi log `Duplicate event`, dừng xử lý | Log cần đủ event ID, thời điểm nhận, source system |
| 1.3b | Hệ thống CVM | Xử lý event mới | Event ID chưa tồn tại | Event được chuyển sang bước match campaign | Event mới cần được ghi nhận để tránh duplicate ở lần sau |
| 1.4 | Hệ thống CVM | Match trigger với campaign đang Active | Trigger code, campaign status, thời gian hiệu lực, logic trigger | Danh sách campaign match hoặc không có campaign match | Chỉ campaign Active và còn hiệu lực mới được match |
| 1.5a | Hệ thống CVM | Xử lý trường hợp không có campaign match | Kết quả match rỗng | Dừng xử lý event | Có thể ghi log phục vụ audit/tuning trigger |
| 1.5b | Hệ thống CVM | Chọn campaign phù hợp để xử lý tiếp | Danh sách campaign match | Campaign/event context được chuyển sang bước lọc khách hàng | Nếu nhiều campaign match, áp dụng rule ưu tiên hoặc xử lý theo từng campaign theo cấu hình |

**Giai đoạn 2: Lọc khách hàng**

| Bước | Tác nhân | Hành động / Xử lý | Input | Output | Quy tắc / Ghi chú |
|---|---|---|---|---|---|
| 2.1 | Hệ thống CVM | Segment Engine kiểm tra điều kiện segment | Customer identifier, payload event, segment rule của campaign | Kết quả khách hàng đạt hoặc không đạt segment | Segment có thể gồm điều kiện profile, thuê bao, gói cước, hành vi, trạng thái dịch vụ |
| 2.2a | Hệ thống CVM | Xử lý khách hàng không đạt segment | Kết quả segment = false | Skip khách hàng, ghi log lý do | Không tiếp tục kiểm tra DNC/Blacklist nếu đã fail segment |
| 2.2b | Hệ thống CVM | Chuyển khách hàng đạt segment sang kiểm tra consent | Kết quả segment = true | Request kiểm tra DNC Flag | Chỉ khách hàng đạt segment mới được xét gửi tin |
| 2.3 | Hệ thống CVM -> BSS | Kiểm tra DNC Flag realtime | Customer identifier | DNC Flag true/false | BSS là nguồn tham chiếu chính cho trạng thái không nhận quảng cáo |
| 2.4a | Hệ thống CVM | Xử lý DNC Flag = true | DNC Flag true | Skip khách hàng, ghi log `DNC blocked` | Tuyệt đối không gửi tin marketing cho khách hàng DNC |
| 2.4b | Hệ thống CVM | Xử lý DNC Flag = false | DNC Flag false | Chuyển sang kiểm tra CVM Blacklist | DNC pass không đồng nghĩa khách hàng được gửi ngay, vẫn phải kiểm tra blacklist nội bộ |
| 2.5 | Hệ thống CVM | Kiểm tra CVM Blacklist theo campaign và kênh gửi | Customer identifier, campaign ID, channel | Kết quả có/không thuộc blacklist | Blacklist CVM có thể áp dụng theo campaign cụ thể hoặc theo kênh cụ thể |
| 2.6a | Hệ thống CVM | Xử lý khách hàng thuộc CVM Blacklist | Kết quả blacklist = true | Skip khách hàng, ghi log `CVM blacklist` | Không gửi qua kênh/campaign bị blacklist |
| 2.6b | Hệ thống CVM | Chuyển khách hàng hợp lệ sang bước throttling | Kết quả blacklist = false | Eligible customer context | Khách hàng đã pass segment, DNC và blacklist |

**Giai đoạn 3: Throttling và render**

| Bước | Tác nhân | Hành động / Xử lý | Input | Output | Quy tắc / Ghi chú |
|---|---|---|---|---|---|
| 3.1 | Hệ thống CVM | Kiểm tra campaign cooldown | Customer identifier, campaign ID, lịch sử gửi campaign | Pass/fail cooldown | Không gửi nếu khách hàng vừa nhận tin cùng campaign trong khoảng cooldown |
| 3.2 | Hệ thống CVM | Kiểm tra daily cap | Customer identifier, tổng số tin đã gửi trong ngày | Pass/fail daily cap | Daily cap giới hạn tổng số tin marketing trong ngày cho một khách hàng |
| 3.3 | Hệ thống CVM | Kiểm tra blackout window | Thời điểm hiện tại, cấu hình blackout | Pass/fail blackout | Nếu nằm trong blackout window, hệ thống skip hoặc delay theo cấu hình |
| 3.4a | Hệ thống CVM | Xử lý trường hợp không pass throttling | Fail một trong các tầng cooldown/daily cap/blackout | Skip hoặc delay, ghi log lý do | Cần phân biệt rõ `Skipped` và `Delayed` để báo cáo đúng |
| 3.4b | Hệ thống CVM | Chuyển khách hàng pass throttling sang render nội dung | Pass cả 3 tầng kiểm soát | Render context | Chỉ render khi khách hàng đủ điều kiện gửi thật |
| 3.5 | Hệ thống CVM | Load template theo kênh ưu tiên | Campaign ID, channel priority, template snapshot | Template content tương ứng | Dùng snapshot đã duyệt để tránh thay đổi nội dung sau khi campaign Active |
| 3.6 | Hệ thống CVM | Inject tham số động từ payload trigger chính | Template content, payload trigger chính | Nội dung đã cá nhân hóa | Nếu thiếu tham số, dùng fallback value nếu được cấu hình; nếu chưa có fallback thì ghi nhận cần xác nhận |
| 3.7 | Hệ thống CVM | Hoàn tất render nội dung gửi | Nội dung đã inject tham số | Message payload sẵn sàng gửi Gateway | Nội dung phải đúng format/ký tự giới hạn của từng kênh |

**Giai đoạn 4: Gửi và xử lý kết quả**

| Bước | Tác nhân | Hành động / Xử lý | Input | Output | Quy tắc / Ghi chú |
|---|---|---|---|---|---|
| 4.1 | Hệ thống CVM -> Gateway | Gửi lệnh qua kênh ưu tiên 1 | Message payload, channel, customer contact, campaign ID | Request gửi tin sang Gateway | Gateway chỉ nhận nội dung đã render, không tự xử lý nghiệp vụ campaign |
| 4.2 | Gateway | Xử lý gửi tin và trả Status Code | Request gửi tin từ CVM | Status Code: Delivered, User Blocked, Expired hoặc Fail khác | Danh sách Status Code chi tiết cần được xác nhận với từng Gateway |
| 4.3a | Hệ thống CVM | Xử lý Status = Delivered | Status Delivered | Ghi log Delivered vào DB | Log gồm campaign, customer, channel, thời điểm gửi, gateway response |
| 4.3b | Hệ thống CVM | Xử lý Status = User Blocked / Expired | Status User Blocked hoặc Expired | Cập nhật `[kênh]_blocked = true` vào profile khách hàng | Sync-back giúp các lần gửi sau không tiếp tục dùng kênh đã bị chặn/hết hiệu lực |
| 4.3c | Hệ thống CVM -> Gateway | Gửi sang kênh fallback tiếp theo nếu có | Fallback channel config, message payload tương ứng | Request gửi qua kênh fallback | Fallback chỉ chạy nếu campaign có cấu hình kênh dự phòng |
| 4.3d | Hệ thống CVM | Xử lý Status = Fail khác | Gateway timeout/lỗi tạm thời/lỗi không xác định | Retry hoặc fallback theo config; ghi log Failed nếu hết retry | Retry không được vượt quá giới hạn cấu hình để tránh spam |
| 4.4 | Hệ thống CVM -> Cơ sở dữ liệu | Ghi log kết quả gửi cuối cùng | Kết quả Delivered/Failed/Skipped/Delayed | Log gửi tin được lưu | Log là nguồn dữ liệu cho báo cáo campaign và audit |
| 4.5 | Hệ thống CVM | Cập nhật số liệu báo cáo campaign | Log gửi tin mới nhất | KPI campaign được cập nhật | Bao gồm sent, delivered, failed, skipped, fallback usage và các chỉ số liên quan |
