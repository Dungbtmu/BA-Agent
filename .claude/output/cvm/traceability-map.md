...
| REQ-CVM-007 | Điều kiện lọc **theo Trigger × Phân khúc × Kênh (v4 — chuyển từ Section 3 xuống Section 4 Message Matrix)** — thuộc tính lấy động theo trigger đã chọn (group theo trigger, không gộp trùng tên), toán tử khai báo thẳng theo thuộc tính, giá trị render theo kiểu (hỗ trợ đơn vị % và GB cho kiểu Số — v4), validate BETWEEN "đến" ≥ "từ"; kế thừa mặc định khi thêm kênh mới; mỗi accordion vẫn giữ logic AND cố định giữa các điều kiện lọc con (không đổi qua các version) — thay đổi thực sự là **phạm vi áp dụng**: từ 1 bộ điều kiện dùng chung mọi kênh (Section 3) sang riêng theo từng kênh (Section 4); **(v4.16) lọc khối hiển thị theo tab biến thể đang mở khi có ≥ 2 Audience Variant** — tránh conflict khi QTV đang soạn 1 phân khúc mà vẫn thấy điều kiện lọc phân khúc khác | UC-CAM-02 | Screen 3, Screen 2B | URD-III-UC-CAM-02, URD-IV-Screen3 (Section 4 STT 4b), URD-IV-Screen2B (Section 4 STT 7) | | Liên quan chặt UC-TRG-05 (nơi khai báo thuộc tính lọc); v3 ở Section 3, v4 chuyển sang Section 4 theo comment nghiệm thu; **v4.5** — rà soát toàn diện đóng nốt 4 vị trí còn sót mô hình cũ (Screen 3 STT 3, UC-CAM-02 Hoạt động, Screen 2B STT 6/7, UC-TRG-05); đồng thời bỏ toàn bộ Reach ước tính (Estimate box, Section 3/6) vì hệ quả trực tiếp — không còn 1 con số ước tính chung đủ chính xác khi điều kiện lọc đã tách theo kênh; **v4.16** — phát hiện qua BA rà soát UI, conflict giữa dropdown gán phân khúc cho biến thể và khối lọc hiện tất cả phân khúc |
...
| REQ-CVM-013 | Xem chi tiết campaign chỉ đọc — toàn bộ cấu hình, nút hành động biến đổi theo trạng thái; **đồng bộ Nhắc lại + bỏ Reach ước tính (v4.12)** — trước đó không hiển thị cấu hình Nhắc lại (thiếu từ v4.1) và còn sót 3 chỗ Reach ước tính (thiếu rà soát từ v4.5) | UC-CAM-04 | Screen 2B | URD-III-UC-CAM-04, URD-IV-Screen2B | | |
| REQ-CVM-014 | Duyệt / Từ chối campaign bởi Admin — phê duyệt chuyển Active, từ chối chuyển Draft kèm lý do bắt buộc; **(v4.15) bấm [Duyệt] mở dialog xác nhận, hệ thống check trùng độ ưu tiên với campaign Active khác ngay lúc đó** — trùng thì dialog đổi nội dung sang cảnh báo (chỉ [Đóng], không có nút xác nhận duyệt); Admin không tự sửa priority tại màn Duyệt; [Từ chối] vẫn là action tự quyết định như bình thường (không bị ép buộc), chỉ là con đường duy nhất để QTV sửa lại priority (thống nhất với REQ-CVM-020) | UC-CAM-05 | Screen Admin | URD-III-UC-CAM-05, URD-IV-Screen-Admin | | v4.15: lỗ hổng thứ 4 của chuỗi chặn trùng độ ưu tiên — trước đó chỉ chặn ở Campaign List/Builder/Priority Matrix, chưa chặn đúng thời điểm Pending → Active |
...
| REQ-CVM-017 | Kích hoạt lại campaign từ Paused — không cần duyệt lại nếu không có cờ lỗi và không có param/điều kiện lọc trigger bị sửa trong lúc Paused (**v4** — nếu có sửa thì chuyển về Chờ duyệt thay vì Active thẳng); khóa nút [Bật] khi còn PARAM_INVALID/FILTER_INVALID **và param/điều kiện lọc vẫn đang Khóa**, resume qua [Sửa] → Draft → gửi duyệt lại; **(v4.13) nhánh Mở khóa** — nếu Admin Mở khóa lại đúng param/điều kiện lọc (không sửa nội dung), [Bật] hoạt động lại và trả về đúng trạng thái gốc trước khi tự Paused (`pre_pause_status`: Pending hoặc Active), không tự động chạy thẳng Active nếu gốc là Pending | UC-CAM-07 | Screen 2, Screen 2B | URD-III-UC-CAM-07, URD-IV-Screen2 (STT 9), URD-IV-Screen2B (STT 3) | | |
...
| REQ-CVM-020 | Độ ưu tiên campaign — mặc định = max priority của campaign Active + 1; **(v4.14) không cho phép 2 campaign Active cùng độ ưu tiên — thống nhất chặn trùng tại 4 nơi**: Campaign List (chặn ngay khi nhập, trước confirm dialog), Campaign Builder (Lưu Nháp vẫn cho trùng, chặn tại Gửi duyệt), Priority Matrix (chặn cứng, disable nút Lưu — hành vi có từ trước), **(v4.15) UC-CAM-05 Phê duyệt** (chặn Pending → Active nếu trùng, Admin chỉ có đường Từ chối — xem REQ-CVM-014); tie-break theo created_at giữ lại chỉ làm phòng vệ race condition; **sửa priority trực tiếp trên Campaign List (v4) — bắt buộc chuyển về Chờ duyệt, khác Priority Matrix (Admin tự sắp xếp không cần duyệt lại)**; **range 1–9999 + thông báo lỗi cụ thể tại Campaign List (v4.11)**, đồng bộ ngưỡng với Campaign Builder | UC-CAM-01, UC-CAM-02, UC-CAM-05, UC-PRIORITY-01 | Screen 2, Screen 3, Screen Settings, Screen Admin | URD-II.6.8, URD-III-UC-CAM-01, URD-III-UC-CAM-02, URD-III-UC-CAM-05, URD-III-UC-PRIORITY-01, URD-IV-Screen2 (STT 7), URD-IV-Screen3 (Section 1 STT 5), URD-IV-Screen-Settings (Tab 3) | | v4: UC-CAM-01 bổ sung vào cột UC vì đã có luồng sửa priority tại Campaign List; v4.14: thống nhất quy tắc chặn trùng, sửa mâu thuẫn Builder ghi "cho phép trùng" trong khi Priority Matrix đã chặn cứng; v4.15: bổ sung UC-CAM-05 — lỗ hổng thứ 4 (Pending → Active chưa từng bị chặn) |
...
| REQ-CVM-035 | Thêm thủ công số vào Blacklist — nhập nhiều số, validate realtime, xử lý case 0 số được lưu mới (không đóng modal, báo lỗi rõ); **chọn nhiều Campaign + nhiều Kênh cùng lúc, tạo N×M bản ghi (v4)**; **Campaign/Kênh bắt buộc — chặn + báo lỗi rõ khi bỏ trống (v4.11)**, sửa mâu thuẫn trước đó (Screen 6B ghi "bỏ qua lặng lẽ") | UC-BL-01 | Screen 6B | URD-III-UC-BL-01, URD-IV-Screen6B | | |
...
| REQ-CVM-037 | Xóa số khỏi Blacklist — confirm dialog, áp dụng cặp campaign-kênh cụ thể hoặc bản ghi toàn hệ thống (**v4**); **2 lớp xóa (v4.9)**: [×] trên chip gỡ đúng 1 tổ hợp, [Xóa] toàn dòng gỡ mọi tổ hợp của số trong phạm vi đang xem; **(v4.17) làm rõ hành vi [×] trên dòng ma trận nhiều-nhiều** (≥2 campaign VÀ ≥2 kênh): 1 chip gộp mọi tổ hợp chứa giá trị đó — xóa chip gỡ TẤT CẢ tổ hợp đó cùng lúc; bổ sung nút [Xem chi tiết tổ hợp] (chỉ hiện trên ma trận nhiều-nhiều) để xóa lẻ đúng 1 tổ hợp cụ thể; text confirm dialog phân biệt 3 case xóa (1 tổ hợp / nhiều tổ hợp qua chip / xóa lẻ qua popover); không ảnh hưởng DNC BSS | UC-BL-03 | Screen 6A | URD-III-UC-BL-03, URD-IV-Screen6A (STT 6, 7, 8, 8b) | | v4.17: phát hiện qua BA rà soát UI — câu URD cũ "click [×] trên chip Kênh hoặc Campaign đều xóa cùng 1 tổ hợp" chỉ đúng khi dòng đơn giản, chưa mô tả case 1 chip đại diện nhiều tổ hợp |
...
| REQ-CVM-054 | Cấu hình Priority Matrix — Admin kéo thả/nhập số sắp xếp thứ tự ưu tiên campaign Active; **chặn cứng khi còn số trùng** (disable nút Lưu — nay là quy tắc chung với REQ-CVM-020, xem v4.14), tiebreak created_at chỉ còn là phòng vệ cuối | UC-PRIORITY-01 | Screen Settings (Tab 3) | URD-III-UC-PRIORITY-01, URD-IV-Screen-Settings (Tab 3) | | |
...
|---|---|---|---|
| 2026-09-15 | MODIFY | REQ-CVM-037 | v4.17: làm rõ hành vi xóa chip Kênh/Campaign trên dòng Blacklist ma trận nhiều-nhiều (≥2 campaign VÀ ≥2 kênh cùng 1 số) — phát hiện qua BA rà soát UI: câu URD cũ mô tả "xóa 1 chip = xóa đúng 1 tổ hợp" chỉ đúng khi dòng đơn giản, chưa xử lý case 1 chip đại diện nhiều tổ hợp cùng lúc. Chốt: xóa chip gỡ TẤT CẢ tổ hợp chứa giá trị đó; bổ sung nút [Xem chi tiết tổ hợp] (chỉ hiện trên ma trận nhiều-nhiều) để xóa lẻ đúng 1 tổ hợp; text confirm dialog phải phân biệt rõ theo từng case xóa |
| 2026-09-08 | MODIFY | REQ-CVM-007 | v4.16: lọc khối "Điều kiện lọc theo Kênh" (Screen 3 Section 4 STT 4b) theo tab biến thể đang mở khi có ≥ 2 Audience Variant — phát hiện qua BA rà soát UI: trước đó khối này luôn hiện đủ mọi phân khúc Section 3, kể cả khi QTV đang mở đúng 1 tab biến thể đã gán riêng 1 phân khúc, gây conflict hiển thị dễ nhầm lẫn sửa sai chỗ. Tab "Tất cả (dự phòng)" và trường hợp chưa bật Variant giữ nguyên hiện tất cả khối |
| 2026-09-07 | MODIFY | REQ-CVM-014, REQ-CVM-020 | v4.15: bổ sung chặn trùng độ ưu tiên tại UC-CAM-05 (Phê duyệt) — lỗ hổng thứ 4 phát hiện qua case BA đặt ra (campaign A Active priority=2, campaign B Pending priority=2, duyệt B → trùng mà không bị chặn vì V4.14 chưa xét thời điểm Pending → Active). Chốt: bấm [Duyệt] mở dialog, hệ thống check trùng ngay lúc đó — trùng thì dialog đổi nội dung sang cảnh báo (chỉ [Đóng]); Admin không tự sửa priority tại màn Duyệt; [Từ chối] không bị ép buộc — vẫn là action tự quyết định bình thường, chỉ là con đường duy nhất để QTV sửa lại priority rồi gửi duyệt lại |
| 2026-08-25 | MODIFY | REQ-CVM-020, REQ-CVM-054, REQ-CVM-061 | v4.14: thống nhất quy tắc chặn trùng độ ưu tiên — phát hiện mâu thuẫn Priority Matrix (đã chặn cứng) vs Campaign Builder (ghi "cho phép trùng") vs Campaign List (chưa quy định) qua rà soát theo yêu cầu BA. Chốt: không cho phép 2 campaign Active cùng độ ưu tiên ở bất kỳ nơi nào sửa (Campaign List, Campaign Builder, Priority Matrix); tie-break created_at giữ lại làm phòng vệ race condition, không còn là cơ chế xử lý trùng chính |
...
| 2026-08-22 | MODIFY | REQ-CVM-017 | v4.13: bổ sung nhánh "Mở khóa param/điều kiện lọc" cho UC-CAM-07 — case BA phát hiện khi team hỏi lại comment nghiệm thu. Campaign Pending bị Khóa param → tự Paused + cờ (đúng policy có sẵn); trước đây UC-CAM-07 chưa có nhánh cho trường hợp Admin Mở khóa lại (không sửa nội dung) — dễ hiểu lầm [Bật] sẽ chạy thẳng Active. Bổ sung: hệ thống lưu trạng thái gốc (pre_pause_status) trước khi tự Paused; Mở khóa xong thì [Bật] trả về đúng trạng thái gốc (Pending nếu gốc là Pending, không tự động Active), vẫn qua confirm dialog. Cập nhật UC-CAM-07 (Tiền/Hậu điều kiện, Hoạt động, Quy tắc nghiệp vụ), policy PARAM_INVALID Khối 3, Screen 2 STT 9, Screen 2B STT 3 |
| 2026-08-22 | MODIFY | REQ-CVM-013 | v4.12: đồng bộ Screen 2B (Campaign Detail View) với các nghiệp vụ đã đổi qua nhiều đợt patch trước — phát hiện qua đánh giá theo yêu cầu BA. Các đợt trước (v4.1 Nhắc lại, v4.5 bỏ Reach) chỉ rà soát kỹ Campaign Builder, bỏ sót Campaign Detail View. Xóa 3 chỗ còn sót Reach ước tính (STT 5, 6, 10); bổ sung hiển thị Nhắc lại vào STT 9; làm rõ STT 10 không còn khối Blackout riêng |
| 2026-08-22 | MODIFY | REQ-CVM-020, REQ-CVM-035 | v4.11: rà soát bổ sung validate cho các field đã update V4.0-V4.10 theo yêu cầu BA. (1) Sửa mâu thuẫn Modal 6B Blacklist — UC-BL-01 ghi Campaign/Kênh bắt buộc nhưng Screen 6B ghi "bỏ qua lặng lẽ, không báo lỗi" — BA xác nhận bắt buộc thật, đổi sang chặn + báo lỗi rõ; (2) bổ sung range 1–9999 + thông báo lỗi cho cột Ưu tiên tại Campaign List (trước chỉ có ở Campaign Builder); (3) làm rõ liên kết đến giới hạn 0–100 cho đơn vị % (đã có sẵn ở Glossary, chưa được trỏ tới ở UC-CAM-02) |
| 2026-08-22 | MODIFY | REQ-CVM-035, REQ-CVM-036 | v4.10: bổ sung ghi chú nối giữa "N×M bản ghi" (UC-BL-01/02, tầng lưu trữ, không đổi) và "gộp 1 dòng theo Số điện thoại" (UC-BL-00/Screen 6A, tầng hiển thị, v4.9) — không phải conflict logic, chỉ thiếu liên kết chéo khiến dễ hiểu nhầm khi đọc riêng lẻ 1 UC. Cập nhật Quy tắc nghiệp vụ UC-BL-01, UC-BL-02, Screen 6B STT 4 |
