...
| REQ-CVM-007 | Điều kiện lọc **theo Trigger × Phân khúc × Kênh (v4 — chuyển từ Section 3 xuống Section 4 Message Matrix)** — thuộc tính lấy động theo trigger đã chọn (group theo trigger, không gộp trùng tên), toán tử khai báo thẳng theo thuộc tính, giá trị render theo kiểu (hỗ trợ đơn vị % và GB cho kiểu Số — v4), validate BETWEEN "đến" ≥ "từ"; kế thừa mặc định khi thêm kênh mới | UC-CAM-02 | Screen 3 | URD-III-UC-CAM-02, URD-IV-Screen3 (Section 4 STT 4b) | | Liên quan chặt UC-TRG-05 (nơi khai báo thuộc tính lọc); v3 ở Section 3, v4 chuyển sang Section 4 theo comment nghiệm thu |
...
| REQ-CVM-034 | Xem danh sách Blacklist — filter Campaign/Kênh/Nguồn/**Phạm vi (v4)**, cột Nguồn phân biệt các cách thêm, đồng bộ 2 chiều với campaign; **hiển thị cả bản ghi Blacklist toàn hệ thống (v4)** | UC-BL-00 | Screen 6A | URD-III-UC-BL-00, URD-IV-Screen6A | | |
...
| REQ-CVM-037 | Xóa số khỏi Blacklist — confirm dialog, áp dụng cặp campaign-kênh cụ thể hoặc bản ghi toàn hệ thống (**v4**), không ảnh hưởng DNC BSS | UC-BL-03 | Screen 6A | URD-III-UC-BL-03, URD-IV-Screen6A (STT 10) | | |
...
| REQ-CVM-068 | Trigger áp dụng cho mẫu tin nhắn — trường mới không bắt buộc tại Template Editor; hiển thị dạng cột Trigger (chip, tối đa 2 + "+N") tại Template List (**v4.3** — ban đầu là nhóm collapsible, BA yêu cầu đổi lại thành cột đơn giản); cho phép Xóa cứng template chưa từng được campaign tham chiếu | UC-TPL-00, UC-TPL-01 | Screen 4A, Screen 4B | URD-III-UC-TPL-00, URD-III-UC-TPL-01, URD-IV-Screen4A | | Mới thêm v4 — comment nghiệm thu "nhóm lại theo trigger, cho phép xóa mẫu tin nhắn"; v4.3 sửa cách hiển thị |
