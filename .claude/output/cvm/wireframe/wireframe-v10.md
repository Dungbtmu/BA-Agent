...
/admin                  → Screen Admin: Duyệt campaign + Cấu hình trigger
```
...
- Thẻ trigger hiển thị `[CODE · Tên trigger ngắn]`; di chuột → hiện đầy đủ: Nguồn / Kiểu chạy / danh sách tham số.
...
- `[Tắt/Bật]`: hộp thoại xác nhận. Nếu trigger đang dùng trong campaign Active → cảnh báo thêm danh sách campaign bị ảnh hưởng.

### Cửa sổ Thêm / Sửa Trigger
...
│ ┌──────────────────┬──────────────────────────┬──────┬───┐ │
│ │ Tên tham số      │ Mô tả                    │Định  │Nguồn│ │
│ │                  │                          │dạng  │    │ │
│ ├──────────────────┼──────────────────────────┼──────┼───┤ │
│ │ [ten_kh_______]  │ [Họ tên đầy đủ của KH__] │[text▾]│[BSS▾][✕]│ │
│ │ [loai_sim_____]  │ [Loại SIM: eSIM/vật lý_] │[text▾]│[BSS▾][✕]│ │
│ └──────────────────┴──────────────────────────┴──────┴───┘ │
│ Định dạng: text / date (DD/MM/YYYY) / number / boolean /   │
│            currency (VND)                                  │
│                                                            │
│                              [Hủy]        [Lưu]           │
└────────────────────────────────────────────────────────────┘
...
- Event source: chỉ BSS hoặc OCS.
- Payload — cột Mô tả: mô tả ngắn ý nghĩa tham số; cột Định dạng: chọn từ danh sách (text / date / number / boolean / currency).
- Priority xung đột không cấu hình ở đây — chỉ cấu hình ở campaign level.
- `[Lưu]` → đóng cửa sổ + thông báo "Đã lưu trigger ✓" + cập nhật bảng theo group đúng loại.
...
| Tab `[Cấu hình Trigger]` | hiện màn Trigger Management (giống Screen 5, đầy đủ quyền Admin) |
