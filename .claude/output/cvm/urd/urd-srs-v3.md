...
Khi tham số của một trigger bị **Khóa** hoặc bị thay đổi (thêm mới, hoặc deploy đổi/gỡ param) — từ **một trong hai nguồn**: (1) Admin thao tác trực tiếp trên UI (UC-TRG-04 — Sửa mô tả, Khóa/Mở), hoặc (2) Dev/SA deploy phiên bản mới (đổi/gỡ param) — hệ thống xử lý như sau. **Lưu ý**: Admin **sửa Tên tham số trên UI KHÔNG kích hoạt cờ** vì cú pháp `{{tham_so}}` giữ nguyên (mã tham chiếu bất biến) — message campaign không gãy; chỉ **Khóa param** (UI) hoặc **deploy đổi/gỡ param** (Dev) mới có thể phát cờ.
...
| C3d | — Cột Giá trị / Bắt buộc | Text | – | – | Kiểu Danh mục: liệt kê giá trị; kiểu Đúng-Sai (boolean): hiển thị "Đúng / Sai"; các kiểu còn lại (số/chuỗi/ngày): "nhập tay" (in nghiêng); có hậu tố "· Bắt buộc" nếu thuộc tính bắt buộc |
...
*Phiên bản: V3.32 | Ngày: 11/08/2026 | Tác giả: Jun (BA)*
