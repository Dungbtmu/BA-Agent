...
│   ├── Thêm / Xóa tham số đầu ra (Quản trị viên hệ thống)
│   └── Thêm / Xóa điều kiện lọc phân khúc (Quản trị viên hệ thống)
│
...
- Các chức năng con: Xem danh sách (bảng phẳng, lọc theo Kiểu chạy: Realtime / Near Realtime / Offline), Xem chi tiết (Định danh + Tham số đầu ra + Điều kiện lọc phân khúc), Khai báo trigger mới (Admin), Thêm/xóa tham số đầu ra (Admin), Thêm/xóa điều kiện lọc phân khúc (Admin)
...
| | Thêm / Xóa tham số đầu ra | X | – |
| | Thêm / Xóa điều kiện lọc phân khúc | X | – |
...
- Admin Hệ thống quản lý trigger trực tiếp trên UI: khai báo trigger mới, thêm/xóa tham số đầu ra, thêm/xóa điều kiện lọc phân khúc. QTV Marketing chỉ xem (danh sách + chi tiết) để tra cứu khi cấu hình campaign — không tạo/sửa/xóa
...
| | Tham số đầu ra của trigger | VIEW, CREATE, DELETE | VIEW |
| | Điều kiện lọc phân khúc của trigger | VIEW, CREATE, DELETE | VIEW |
...
| **Hoạt động** | 1. Người dùng click [Bật] <br>1a. Hệ thống chuyển trạng thái → Active ngay (không cần confirm); toast "Campaign đã kích hoạt lại"; button chuyển từ [Bật] → [Dừng] <br>**[Exception — còn cờ PARAM_INVALID]**: Nút [Bật] disabled; hover → tooltip "Campaign đang có tham số không hợp lệ do trigger đã thay đổi — vui lòng vào [Sửa] để cập nhật nội dung message trước khi gửi duyệt lại"; QTV chỉ còn lựa chọn [Sửa], không có đường tắt bật thẳng <br>**[Exception — còn cờ FILTER_INVALID]**: Nút [Bật] disabled; hover → tooltip "Campaign đang có điều kiện lọc không hợp lệ do trigger đã thay đổi thuộc tính lọc — vui lòng vào [Sửa] để cập nhật điều kiện lọc trước khi gửi duyệt lại"; xử lý giống PARAM_INVALID — chỉ resume qua [Sửa] → Draft → gửi duyệt lại |
...
| **Quy tắc nghiệp vụ** | - Nút [Gửi duyệt] chỉ active khi tổng số issue blocking = 0 <br>- Các issue blocking bắt buộc: Tên campaign trống; Thời gian hiệu lực chưa chọn; **Ngày kết thúc ở trong quá khứ** (endDate < ngày hiện tại); Không có trigger nào; Tất cả kênh đều chưa có nội dung; Banner bật mà chưa upload ảnh 16:9; Blacklist bật mà chưa chọn tệp; Whitelist bật mà chưa chọn tệp; **còn cờ `PARAM_INVALID`**; **còn cờ `FILTER_INVALID`** (cả hai xem Quy tắc nghiệp vụ chung — Khối 3) <br>- **Cờ `PARAM_INVALID` là blocking issue độc lập, không phụ thuộc QTV đã sửa trường nào khác**: dù QTV sửa các trường không liên quan (tên campaign, thời gian hiệu lực...) và không đụng đến nội dung message chứa param lỗi, nút [Gửi duyệt] vẫn bị vô hiệu hóa cho đến khi QTV thực sự cập nhật đúng nội dung message để thay thế/xóa param đã bị xóa hoặc đổi tên — hệ thống chỉ tự xóa cờ khi phát hiện nội dung message không còn tham chiếu param lỗi nữa <br>- **Cờ `FILTER_INVALID` cũng là blocking issue độc lập**: nút [Gửi duyệt] bị vô hiệu hóa cho đến khi QTV cập nhật hoặc gỡ điều kiện lọc tham chiếu thuộc tính bị Admin xóa/đổi ở Section 3; hệ thống chỉ tự xóa cờ khi điều kiện lọc lỗi không còn; nếu campaign dính cả 2 cờ thì phải xử lý xong cả hai <br>- Sau khi gửi duyệt, campaign không thể sửa cho đến khi Admin từ chối trả về Draft |
...
| **Mục tiêu** | Cho phép người dùng xem thông tin định danh, tham số đầu ra và điều kiện lọc phân khúc của một trigger; Admin có thể thêm/xóa tham số và điều kiện lọc |
...
### UC-TRG-05: Thêm / Xóa điều kiện lọc phân khúc

| Nội dung | Mô tả |
|----------|-------|
| **Tên** | Thêm / Xóa điều kiện lọc phân khúc |
| **Mục tiêu** | Cho phép Admin khai báo các thuộc tính dùng để lọc phân khúc khách hàng khi tạo campaign; danh sách thuộc tính khả dụng ở Section 3 Campaign Builder lấy động từ đây |
| **Tác nhân** | Admin Hệ thống |
| **Trigger** | Admin click [+ Thêm điều kiện lọc] hoặc icon [Xóa] trong Nhóm C của modal UC-TRG-02, hoặc khai báo ngay khi tạo trigger (UC-TRG-03 bước 6) |
| **Tiền điều kiện** | - Admin đã đăng nhập <br>- Trigger đã được tạo (UC-TRG-03) |
| **Hậu điều kiện** | - Danh sách điều kiện lọc của trigger được cập nhật; thay đổi phản ánh ngay trong accordion "Điều kiện lọc thêm" của Campaign Builder (với trigger đó) |
| **Hoạt động** | **Thêm điều kiện lọc:** <br>1. Admin click [+ Thêm điều kiện lọc] → inline form hiển thị ngay trong modal <br>2. Admin nhập **Tên thuộc tính** (bắt buộc; ngôn ngữ nghiệp vụ; ví dụ: "Phân khúc tuổi") <br>3. Admin chọn **Kiểu dữ liệu** (bắt buộc; một trong: Danh mục / Chuỗi / Số nguyên / Số thập phân / Số thực / Đúng-Sai / Ngày / Ngày giờ) <br>4. Admin tick **Toán tử hỗ trợ** (bắt buộc, chọn ít nhất 1; hiển thị nhãn tiếng Việt kèm ký hiệu gốc: Bằng `=`, Khác `!=`, Lớn hơn `>`, Nhỏ hơn `<`, Lớn hơn hoặc bằng `>=`, Nhỏ hơn hoặc bằng `<=`, Trong khoảng `BETWEEN`, Thuộc danh sách `IN`, Không thuộc danh sách `NOT IN`, Có chứa `CONTAINS`, Sau ngày `AFTER`, Trước ngày `BEFORE`, Bỏ trống `IS NULL`, Có giá trị `IS NOT NULL`) <br>5. Admin tick **Bắt buộc** nếu QTV phải điền thuộc tính này trước khi lưu campaign (mặc định: không bắt buộc) <br>6. **[Chỉ khi Kiểu = Danh mục]** Admin nhập **Danh sách giá trị** (các giá trị cách nhau bằng dấu phẩy) <br>7. Admin click [Lưu] → hệ thống validate → thêm vào bảng; toast "Đã thêm điều kiện lọc ✓" <br>**Xóa điều kiện lọc:** <br>1. Admin hover vào dòng → icon [Xóa] hiển thị <br>2. Admin click [Xóa] → hệ thống mở **dialog xác nhận** (không xóa ngay): tiêu đề "Xóa điều kiện lọc?"; nội dung nêu tên thuộc tính đang xóa + mã trigger; nếu có campaign đang dùng trigger này thì hiển thị cảnh báo chung kèm **danh sách campaign bị ảnh hưởng** (tên + mã, không lọc theo trạng thái) với câu "N chiến dịch đang dùng trigger này sẽ bị đánh dấu cần rà soát lại điều kiện lọc; chiến dịch đang chạy sẽ tự chuyển Tạm dừng và phải cập nhật điều kiện lọc trước khi bật lại"; nếu chưa campaign nào dùng thì báo "xóa không ảnh hưởng chiến dịch đang chạy" <br>3. Admin click [Xác nhận xóa] → hệ thống xóa; toast "Đã xóa điều kiện lọc"; các campaign bị ảnh hưởng được gắn cờ `FILTER_INVALID` theo policy Khối 3. Click [Hủy] → đóng dialog, không xóa <br>**[Exception — Tên thuộc tính trống]**: Inline error "Bắt buộc" <br>**[Exception — Tên thuộc tính trùng]**: Inline error "Thuộc tính đã tồn tại" <br>**[Exception — Chưa tick toán tử]**: Inline error "Chọn ít nhất 1 toán tử" <br>**[Exception — Kiểu Danh mục nhưng chưa nhập giá trị]**: Inline error "Kiểu danh mục cần danh sách giá trị" |
| **Quy tắc nghiệp vụ** | - Tên thuộc tính là định danh duy nhất trong phạm vi 1 trigger (không phân biệt hoa thường) <br>- **Toán tử khai báo thẳng per thuộc tính, không suy máy móc từ kiểu dữ liệu** — cùng kiểu Số nhưng mỗi thuộc tính có thể hỗ trợ bộ toán tử khác nhau tùy nghiệp vụ (ví dụ "Số dư" chỉ cần `<, <=, =, >`; "Tỷ lệ dùng" cần thêm `BETWEEN`) <br>- Admin chỉ nhập tên nghiệp vụ; hệ thống tự sinh mã kỹ thuật (techName) từ tên để Dev tham chiếu — Admin không nhập techName <br>- Danh sách giá trị chỉ áp dụng cho kiểu Danh mục (enum); các kiểu còn lại QTV nhập giá trị trực tiếp khi lọc, không cần khai báo danh sách sẵn <br>- Sau khi thêm/xóa điều kiện lọc, thay đổi phản ánh ngay ở accordion "Điều kiện lọc thêm" trong Campaign Builder của các campaign dùng trigger đó — với campaign đang chạy, xem policy `FILTER_INVALID` tại Quy tắc nghiệp vụ chung Khối 3 <br>- **Xóa điều kiện lọc phải qua dialog xác nhận** (không xóa ngay): dialog nêu rõ campaign nào đang dùng trigger để Admin lường trước phạm vi ảnh hưởng trước khi xác nhận. Thêm điều kiện lọc mới không cần confirm (không phá vỡ campaign hiện có). Nội dung dialog chỉ cảnh báo chung + danh sách campaign, không liệt kê lại từng thuộc tính của trigger (tránh dài dòng khi trigger có nhiều thuộc tính) |
...
Khi tham số của một trigger bị thay đổi (thêm, đổi tên, hoặc xóa param) — từ **một trong hai nguồn**: (1) Admin sửa trực tiếp trên UI (UC-TRG-04), hoặc (2) Dev/SA deploy phiên bản mới — hệ thống xử lý như sau:

- **Điều kiện áp dụng cờ `PARAM_INVALID`** — phải thỏa đồng thời cả 2 điều kiện, không áp dụng tràn lan cho mọi campaign: <br>　(a) Campaign đang **sử dụng trigger** vừa bị thay đổi tham số (bất kỳ trigger nào trong cấu hình campaign, kể cả Advanced mode nhiều trigger); <br>　(b) Nội dung message của campaign đó (Message Matrix) có **tham chiếu trực tiếp đến đúng param bị xóa hoặc đổi tên** (ví dụ dùng cú pháp `{{ten_kh}}` mà `ten_kh` là param bị xóa/đổi tên) <br>　→ Campaign dùng trigger đó nhưng nội dung message **không tham chiếu param bị ảnh hưởng** (ví dụ chỉ dùng `{{so_du}}` trong khi param bị xóa là `ten_kh`) thì **không bị gắn cờ**, không bị chuyển trạng thái — vì nội dung tin nhắn không thực sự hỏng <br>　→ Param bị **thêm mới** không kích hoạt cờ này với bất kỳ campaign nào (thêm param không phá vỡ message hiện có, không có gì bị "invalid")
...
- Hệ thống gửi thông báo nội bộ đến QTV sở hữu campaign **một lần** ngay tại thời điểm phát hiện, liệt kê: tên campaign bị ảnh hưởng + param nào đã thay đổi. Đây chỉ là cảnh báo tức thời — cờ `PARAM_INVALID` bản thân nó tồn tại persistent trên campaign (không biến mất nếu QTV bỏ qua thông báo); nếu QTV không xử lý ngay mà sau đó mới vào [Sửa] campaign, hệ thống **hiển thị lại** cảnh báo dưới dạng banner ngay trong Campaign Builder (xem UC-CAM-03) — đảm bảo QTV luôn biết chính xác cần sửa gì dù có bỏ lỡ thông báo ban đầu
...
**Yêu cầu quy trình:** Trước khi deploy thay đổi param, Dev/SA phải thông báo cho QTV/BA tối thiểu 1 sprint (≥ 1 tuần) để QTV chuẩn bị cập nhật campaign. Với thay đổi param trực tiếp trên UI, Admin nên thông báo QTV/BA trước khi xóa/đổi param đang được nhiều campaign sử dụng, hạn chế số campaign bị Paused đồng loạt.

---

**Policy thay đổi Điều kiện lọc phân khúc (FILTER_INVALID):**

Khi Admin sửa điều kiện lọc của một trigger (xóa thuộc tính, đổi/bỏ giá trị danh mục đang được lọc, hoặc bỏ toán tử làm điều kiện cũ không còn hợp lệ), hệ thống áp dụng cơ chế song song với `PARAM_INVALID`. Lý do tách riêng: điều kiện lọc quyết định **tập khách hàng nhận tin** — điều kiện lọc hỏng làm audience bị tính sai âm thầm (gửi nhầm người hoặc bỏ sót), nội dung tin nhắn nhìn vẫn bình thường nên rủi ro cao hơn và cần chặn sớm.

- **Điều kiện áp dụng cờ `FILTER_INVALID`** — phải thỏa đồng thời cả 2 điều kiện: <br>　(a) Campaign đang **sử dụng trigger** vừa bị Admin sửa điều kiện lọc (kể cả Advanced mode nhiều trigger); <br>　(b) Campaign đó **thực sự có điều kiện lọc** ở Section 3 tham chiếu đúng thuộc tính bị xóa/đổi, hoặc lọc theo giá trị danh mục vừa bị bỏ, hoặc dùng toán tử vừa bị gỡ <br>　→ Campaign dùng trigger đó nhưng **không lọc theo thuộc tính bị ảnh hưởng** thì **không bị gắn cờ**, không đổi trạng thái <br>　→ Thuộc tính lọc bị **thêm mới** không kích hoạt cờ này với bất kỳ campaign nào (thêm thuộc tính không phá vỡ điều kiện lọc hiện có)
...
- Hệ thống gửi thông báo nội bộ đến QTV sở hữu campaign **một lần** ngay tại thời điểm phát hiện, liệt kê: tên campaign bị ảnh hưởng + thuộc tính lọc nào đã thay đổi. Cờ tồn tại persistent trên campaign; nếu QTV không xử lý ngay, hệ thống **hiển thị lại** cảnh báo dưới dạng banner đỏ ngay trong Campaign Builder khi QTV mở [Sửa] (giống banner PARAM_INVALID tại UC-CAM-03)
- Campaign bị chuyển Paused do policy này: nút [Bật] (kích hoạt lại trực tiếp — UC-CAM-07) bị khóa vĩnh viễn; QTV bắt buộc dùng [Sửa] → Campaign Builder → cập nhật hoặc gỡ điều kiện lọc lỗi ở Section 3 → [Lưu nháp] → campaign về Draft (cờ `FILTER_INVALID` được xóa) → [Gửi duyệt] lại → Pending → Admin duyệt → Active
...
**Yêu cầu quy trình với Admin:** Trước khi xóa/đổi thuộc tính lọc đang được nhiều campaign sử dụng, Admin nên thông báo QTV/BA để chuẩn bị cập nhật campaign, hạn chế số campaign bị Paused đồng loạt.
...
| **Hoạt động** | 1. Người dùng nhập số điện thoại (bắt buộc; hỗ trợ nhập nhiều số phân cách bằng dấu phẩy hoặc xuống dòng) <br>2. Người dùng chọn Campaign (bắt buộc) và Kênh (bắt buộc) từ dropdown <br>3. Hệ thống validate realtime: số hợp lệ màu xanh, sai định dạng màu đỏ kèm tooltip <br>4. Người dùng click [Thêm]: hệ thống lọc bỏ số sai định dạng và số đã trùng trong blacklist của cùng campaign-kênh, lưu các số còn lại → đóng modal + toast "Đã thêm X số vào blacklist ✓" (X = số lượng thực sự lưu mới) <br>**[Exception — không có số hợp lệ nào ngay khi nhập]**: Nút [Thêm] disabled; hiển thị "Chưa có số hợp lệ để thêm" <br>**[Exception — bấm Thêm nhưng không có số nào được lưu mới]**: Xảy ra khi toàn bộ số hợp lệ về định dạng đều đã trùng trong blacklist của cùng campaign-kênh; hệ thống **không đóng modal**, hiển thị thông báo lỗi đỏ ngay trong modal: "Toàn bộ số đã nhập đều đã có trong blacklist — không có số nào được thêm"; QTV có thể sửa lại danh sách số hoặc tự đóng modal |
| **Quy tắc nghiệp vụ** | - Số hợp lệ: 10 chữ số, bắt đầu bằng 0 <br>- Số sai định dạng bị bỏ qua, không block toàn bộ thao tác; hiển thị số lượng bị bỏ qua trong toast <br>- Số đã có trong blacklist của cùng campaign-kênh: bỏ qua (không báo lỗi riêng cho từng số, không tạo bản ghi trùng) <br>- **Trường hợp 0 số được lưu mới** (toàn bộ số hợp lệ về định dạng đều trùng): không tính là thao tác thành công — không đóng modal, không hiện toast "Đã thêm 0 số"; thay vào đó hiển thị lỗi ngay trong modal để QTV biết rõ lý do và có thể sửa lại |
...
| 6 | Section 3 — Audience | Read-only | – | – | Danh sách phân khúc đã chọn (tag), logic phân khúc, số KH ước tính. **Điều kiện lọc**: hiển thị dạng danh sách dọc, mỗi dòng gắn nhãn tên phân khúc ở đầu để phân biệt điều kiện đó thuộc phân khúc nào; mỗi điều kiện hiển thị dạng chip gồm nội dung điều kiện **kèm badge mã trigger nguồn** để biết thuộc tính đó thuộc trigger nào (nhất quán với Campaign Builder Section 3 — nơi thuộc tính lọc lấy động theo trigger, group theo trigger) — ví dụ "**[ARPU cao]** [Phân khúc tuổi thuộc 25-34 · E01] [Gói cước hiện tại thuộc D200 · E01]", "**[Gen Z User]** [Loại SIM thuộc eSIM · E02] [Loại thiết bị thuộc ANDROID · E02]"; các điều kiện trong cùng nhãn phân khúc quan hệ AND (đúng như đã cấu hình ở Campaign Builder); ở Advanced mode nhiều trigger, badge trigger giúp phân biệt các thuộc tính trùng tên giữa các trigger; phân khúc không có điều kiện lọc riêng thì không xuất hiện dòng nào cho phân khúc đó (không hiển thị nhãn rỗng) |
| 7 | Section 4 — Message Matrix | Tab + Read-only | – | Tab đầu tiên có nội dung | Tab mỗi kênh (Push/Zalo/SMS/USSD/Banner/Email); thứ tự tab theo thứ tự kênh thêm vào khi tạo; click tab → xem nội dung từng trigger card; nội dung hiển thị dạng card chỉ đọc (tiêu đề, nội dung, ảnh thumbnail nếu có); **Tab kênh chưa có nội dung**: vẫn hiển thị tab nhưng bên trong ghi "Chưa có nội dung" <br>**Biến thể đối tượng (chỉ đọc):** <br>• **Có biến thể**: bên trong mỗi trigger card hiển thị hàng tab biến thể ("Tất cả (dự phòng)" / "Tên phân khúc" / ...); tab mặc định = Biến thể 1; click tab → nội dung bên dưới cập nhật; tất cả chỉ đọc <br>• **Không có biến thể** (AND mode hoặc chỉ có 1 biến thể mặc định): không hiển thị hàng tab, hiển thị thẳng nội dung <br>• **Số biến thể per kênh độc lập**: mỗi kênh có thể có số biến thể khác nhau (ví dụ Push 3 biến thể, SMS chỉ 1); kênh có 1 biến thể → không có hàng tab; kênh có ≥2 biến thể → có hàng tab <br>• **Số biến thể per trigger độc lập**: trong cùng một kênh, T1 có thể có 3 biến thể trong khi T2 chỉ có 1; mỗi trigger card tự kiểm tra và render hàng tab độc lập <br>• **Reset khi switch kênh**: chuyển sang kênh khác → tab biến thể reset về Biến thể 1 ("Tất cả (dự phòng)"); tránh giữ index cũ sang kênh có ít biến thể hơn |
...
| 5 | Điều kiện lọc per phân khúc | Accordion trong từng thẻ | Không | Thu gọn | Mỗi thẻ có accordion [+ Thêm lọc] / [Sửa] riêng; mở → hiện các dòng điều kiện; mỗi dòng: [Thuộc tính ▾] [Toán tử ▾] [Giá trị] [×]; **Thuộc tính: danh sách động theo trigger đã chọn ở Section 2** (không còn là danh sách cố định toàn hệ thống) — mỗi trigger có bộ thuộc tính lọc riêng do Admin khai báo tại Trigger Admin (xem UC-TRG-05), ví dụ trigger "Data ngày sắp hết" có % data đã dùng, gói cước, số ngày còn lại chu kỳ; trigger "SIM kích hoạt lần đầu" có loại SIM, phân khúc tuổi, nguồn kích hoạt <br>**Advanced mode chọn nhiều trigger — group theo trigger**: dropdown Thuộc tính chia nhóm theo từng trigger (optgroup có tiêu đề là mã trigger); **thuộc tính trùng tên giữa các trigger được giữ riêng theo từng trigger, KHÔNG gộp làm một** — mỗi điều kiện lọc đã chọn hiển thị kèm badge mã trigger nguồn để phân biệt rõ nó thuộc trigger nào (kể cả khi accordion đã thu gọn); lý do: cùng tên "Gói cước" nhưng của 2 trigger khác nhau có thể mang giá trị/ngữ nghĩa khác nhau <br>**Toán tử: khai báo thẳng theo từng thuộc tính** (không suy máy móc từ kiểu dữ liệu) — dropdown toán tử của mỗi dòng chỉ hiển thị đúng bộ toán tử Admin đã khai báo cho thuộc tính đó (ví dụ "Số dư" chỉ có Bằng/Nhỏ hơn/Lớn hơn; "Tỷ lệ dùng" có thêm Trong khoảng); nhãn toán tử hiển thị tiếng Việt (Bằng, Khác, Lớn hơn, Nhỏ hơn, Lớn hơn hoặc bằng, Nhỏ hơn hoặc bằng, Trong khoảng, Thuộc danh sách, Không thuộc danh sách, Có chứa, Sau ngày, Trước ngày, Bỏ trống, Có giá trị); đổi thuộc tính sang thuộc tính khác kiểu → toán tử tự reset về giá trị hợp lệ đầu tiên <br>**Ô giá trị: render theo kiểu dữ liệu của thuộc tính** — kiểu Danh mục (enum) → dropdown chọn từ danh sách Admin khai báo; kiểu Đúng-Sai (boolean) → dropdown 2 giá trị (Đúng / Sai); kiểu Số (integer/decimal/float) và Chuỗi (string) → nhập trực tiếp; kiểu Ngày (date) và Ngày giờ (datetime) → date/datetime picker; toán tử **Trong khoảng (BETWEEN)** → 2 ô nhập (từ – đến); toán tử **Bỏ trống / Có giá trị (IS NULL / IS NOT NULL)** → không có ô giá trị <br>**Chưa chọn trigger nào ở Section 2**: accordion bị vô hiệu hóa ở mọi thẻ phân khúc, hiển thị text mờ "Chọn trigger ở mục 2 để xem điều kiện lọc khả dụng", ẩn nút [+ Thêm lọc] <br>**Đã chọn trigger nhưng không trigger nào khai báo điều kiện lọc** (tất cả trigger đã chọn đều có danh sách thuộc tính lọc rỗng): accordion cũng bị vô hiệu hóa, hiển thị text mờ "Trigger đã chọn chưa khai báo điều kiện lọc nào — phân khúc dùng toàn bộ audience của trigger", ẩn nút [+ Thêm lọc] — không cho thêm dòng điều kiện rỗng không chọn được thuộc tính. Advanced mode nhiều trigger mà **một phần** trigger có điều kiện lọc, phần còn lại không: dropdown Thuộc tính chỉ hiển thị optgroup của các trigger **có** thuộc tính lọc (bỏ trigger rỗng khỏi dropdown), accordion vẫn bật bình thường <br>Nhiều điều kiện trong cùng thẻ quan hệ AND; [Áp dụng] cập nhật số KH sau lọc trong thẻ; mỗi thẻ phân khúc tự cấu hình giá trị lọc độc lập dù cùng chọn chung 1 thuộc tính từ danh sách khả dụng |
...
| 4 | Nút [Lưu Template] | Button | – | – | Validate tên bắt buộc; kiểm tra toggle Inactive vs Active campaign (xem STT 3); **block lưu nếu có tab kênh trống** — inline error trên từng tab thiếu nội dung: "Kênh [X] chưa có nội dung — vui lòng điền trước khi lưu"; **Loading state**: nút disabled + spinner trong lúc call API; khi API trả về thành công → toast "Đã lưu template ✓" + navigate /templates; nếu API lỗi → toast "Lưu thất bại — vui lòng thử lại" + nút trở về enabled |
...
| C1d | — Cột Giá trị / Bắt buộc | Text | – | – | Kiểu Danh mục: liệt kê giá trị; kiểu khác: "nhập tay"; hậu tố "· Bắt buộc" nếu thuộc tính bắt buộc |
...
| 4 | Xử lý số sai định dạng và số trùng | – | – | – | Khi nhấn [Thêm]: số sai định dạng bị lọc bỏ (không thêm, không chặn toàn bộ thao tác); số đã tồn tại trong blacklist với cùng Campaign + Kênh cũng bị lọc bỏ (không tạo bản ghi trùng, không báo lỗi riêng cho từng số) — chỉ các số hợp lệ và chưa trùng mới được lưu |
| 5 | Nút [Hủy] | Button | – | – | Đóng modal, không lưu; không cần confirm khi hủy |
| 6 | Nút [Thêm] | Button | – | – | Disabled khi ô Số điện thoại chưa có số hợp lệ nào (theo định dạng); **Khi nhấn**: (1) Campaign — chưa chọn thì bỏ qua lặng lẽ; (2) Kênh — chưa chọn thì bỏ qua lặng lẽ; (3) hệ thống lọc số sai định dạng + số trùng, lưu các số còn lại; nút bị khoá trong lúc xử lý → **Khi có ít nhất 1 số được lưu mới**: đóng modal + thêm các dòng mới lên đầu bảng danh sách + toast "Đã thêm X số vào blacklist ✓" (X = số lượng thực sự lưu mới); **Khi 0 số được lưu mới** (toàn bộ số hợp lệ định dạng đều đã trùng): modal **không đóng**, hiển thị lỗi đỏ ngay trong modal "Toàn bộ số đã nhập đều đã có trong blacklist — không có số nào được thêm", nút mở khóa lại; **Khi hệ thống lỗi**: toast "Thêm thất bại — vui lòng thử lại" + modal giữ nguyên + nút mở khóa lại |
...
| C2 | Inline form thêm điều kiện lọc _(T-FILTER)_ | Form | – | Ẩn | Hiện khi nhấn [+ Thêm điều kiện lọc]; gồm: **Tên thuộc tính** (bắt buộc; ngôn ngữ nghiệp vụ; placeholder "vd: Phân khúc tuổi"), **Kiểu dữ liệu** (select 8 kiểu: Danh mục / Chuỗi / Số nguyên / Số thập phân / Số thực / Đúng-Sai / Ngày / Ngày giờ), **Bắt buộc** (checkbox), **Toán tử hỗ trợ** (nhóm nút tick nhiều — nhãn tiếng Việt kèm ký hiệu gốc: Bằng `=`, Khác `!=`, Lớn hơn `>`, Nhỏ hơn `<`, Lớn hơn hoặc bằng `>=`, Nhỏ hơn hoặc bằng `<=`, Trong khoảng `BETWEEN`, Thuộc danh sách `IN`, Không thuộc danh sách `NOT IN`, Có chứa `CONTAINS`, Sau ngày `AFTER`, Trước ngày `BEFORE`, Bỏ trống `IS NULL`, Có giá trị `IS NOT NULL`), **Danh sách giá trị** (chỉ hiện khi Kiểu = Danh mục; các giá trị cách nhau bằng dấu phẩy); nút [Lưu] + [Hủy]; validate inline trước khi lưu |
| C3 | Bảng điều kiện lọc | Table (4 cột + action) | – | – | Liệt kê toàn bộ điều kiện lọc; không phân trang |
| C3a | — Cột Thuộc tính | Text | – | – | Tên nghiệp vụ (dòng trên) + mã kỹ thuật tự sinh (dòng dưới, font mono, màu mờ — để trao đổi với Dev) |
...
| C3d | — Cột Giá trị / Bắt buộc | Text | – | – | Kiểu Danh mục: liệt kê giá trị; kiểu khác: "nhập tay" (in nghiêng); có hậu tố "· Bắt buộc" nếu thuộc tính bắt buộc |
| C3e | — Cột Hành động | Icon button | – | – | Icon [Xóa] hiện khi hover dòng; nhấn → mở **dialog xác nhận** C4 (không xóa ngay) |
| C4 | Dialog xác nhận xóa điều kiện lọc | Dialog | – | Ẩn | Hiện khi nhấn icon [Xóa] ở C3e; tiêu đề "Xóa điều kiện lọc?"; nội dung: "Bạn đang xóa điều kiện lọc **[tên thuộc tính]** khỏi trigger `[mã trigger]`"; nếu **có campaign đang dùng trigger** → khối cảnh báo màu hổ phách: "**N chiến dịch** đang dùng trigger này sẽ bị đánh dấu cần rà soát lại điều kiện lọc" + danh sách campaign (tên đậm + mã mờ, không phân trang) + dòng "Chiến dịch đang chạy sẽ tự chuyển **Tạm dừng** và phải cập nhật điều kiện lọc trước khi bật lại"; nếu **chưa campaign nào dùng** → text mờ "Chưa có chiến dịch nào dùng trigger này — xóa không ảnh hưởng chiến dịch đang chạy"; nút [Hủy] (outline) + [Xác nhận xóa] (đỏ); [Xác nhận xóa] → xóa dòng khỏi bảng, toast "Đã xóa điều kiện lọc", campaign bị ảnh hưởng gắn cờ `FILTER_INVALID` (policy Khối 3); [Hủy] hoặc Escape → đóng, không xóa |
| C4 | Trạng thái rỗng | Text | – | – | "Chưa có điều kiện lọc nào" (in nghiêng, màu xám) khi bảng trống |
...
| Danh sách giá trị | Bắt buộc khi Kiểu = Danh mục | "Kiểu danh mục cần danh sách giá trị" |
...
| N6a | Inline form thêm điều kiện lọc | – | – | – | Hiện khi click [+ Thêm điều kiện lọc]: giống form T-FILTER (Tên thuộc tính + Kiểu dữ liệu + Bắt buộc + Toán tử hỗ trợ tick nhiều + Danh sách giá trị nếu Danh mục); nút [Lưu] và [Hủy]; validate như T-FILTER |
...
*Phiên bản: V3.27 | Ngày: 15/07/2026 | Tác giả: Jun (BA)*
