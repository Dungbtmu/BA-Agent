...
| **Hoạt động** | 1. Hệ thống load Campaign Builder với toàn bộ dữ liệu đã lưu pre-filled <br>2. QTV chỉnh sửa các section cần thiết (logic giống UC-CAM-02) <br>3. QTV lưu nháp hoặc gửi duyệt (logic giống UC-CAM-02) <br>**[Exception — trigger đã chọn bị tắt]**: Chip trigger đó highlight đỏ với cảnh báo "Trigger đã bị tắt — vui lòng chọn trigger khác" <br>**[Exception — campaign còn cờ PARAM_INVALID]**: Ngay khi Campaign Builder tải xong, hiển thị banner cảnh báo màu đỏ ở đầu trang: "⚠ Campaign đang có tham số không hợp lệ do trigger [tên trigger] đã thay đổi tham số [tên param] — vui lòng cập nhật nội dung message trước khi gửi duyệt lại"; banner hiển thị lại **mỗi lần** QTV mở campaign này để sửa, không phụ thuộc việc QTV đã đọc thông báo nội bộ ban đầu hay chưa; banner tự ẩn ngay khi QTV sửa xong nội dung tham chiếu param lỗi và [Lưu nháp] (cờ được xóa) |
...
| **Quy tắc nghiệp vụ** | - Nút [Gửi duyệt] chỉ active khi tổng số issue blocking = 0 <br>- Các issue blocking bắt buộc: Tên campaign trống; Thời gian hiệu lực chưa chọn; **Ngày kết thúc ở trong quá khứ** (endDate < ngày hiện tại); Không có trigger nào; Tất cả kênh đều chưa có nội dung; Banner bật mà chưa upload ảnh 16:9; Blacklist bật mà chưa chọn tệp; Whitelist bật mà chưa chọn tệp; **còn cờ `PARAM_INVALID`** (xem Quy tắc nghiệp vụ chung — Khối 3) <br>- **Cờ `PARAM_INVALID` là blocking issue độc lập, không phụ thuộc QTV đã sửa trường nào khác**: dù QTV sửa các trường không liên quan (tên campaign, thời gian hiệu lực...) và không đụng đến nội dung message chứa param lỗi, nút [Gửi duyệt] vẫn bị vô hiệu hóa cho đến khi QTV thực sự cập nhật đúng nội dung message để thay thế/xóa param đã bị xóa hoặc đổi tên — hệ thống chỉ tự xóa cờ khi phát hiện nội dung message không còn tham chiếu param lỗi nữa <br>- Sau khi gửi duyệt, campaign không thể sửa cho đến khi Admin từ chối trả về Draft |
...
| 7 | Banner cảnh báo PARAM_INVALID | Alert banner (đỏ, full-width) | – | Ẩn | Chỉ hiển thị khi campaign đang sửa còn cờ `PARAM_INVALID`; nằm ngay dưới Header, phía trên Section 1; nội dung: "⚠ Campaign đang có tham số không hợp lệ do trigger [tên trigger] đã thay đổi tham số [tên param] — vui lòng cập nhật nội dung message trước khi gửi duyệt lại"; hiển thị lại **mỗi lần** mở campaign này để sửa (không phụ thuộc thông báo nội bộ ban đầu đã đọc hay chưa — xem Quy tắc nghiệp vụ chung Khối 3); tự ẩn ngay khi QTV sửa xong nội dung tham chiếu param lỗi và [Lưu Nháp] thành công (cờ được xóa) |
...
*Phiên bản: V3.5 | Ngày: 19/06/2026 | Tác giả: Jun (BA)*
