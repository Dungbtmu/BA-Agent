...
| 29/06/2026 | Jun | Screen 2B (STT 7) | U | Bổ sung hiển thị Biến thể đối tượng trong Campaign Detail View: khi campaign có biến thể, bên trong mỗi trigger card hiển thị hàng tab biến thể (Tất cả dự phòng / tên phân khúc); tab mặc định = Biến thể 1; click tab → nội dung cập nhật; không có biến thể → hiển thị thẳng nội dung, không có tab. | V3.7 |
...
| 7 | Section 4 — Message Matrix | Tab + Read-only | – | Tab đầu tiên có nội dung | Tab mỗi kênh (Push/Zalo/SMS/USSD/Banner/Email); thứ tự tab theo thứ tự kênh thêm vào khi tạo; click tab → xem nội dung từng trigger card; nội dung hiển thị dạng card chỉ đọc (tiêu đề, nội dung, ảnh thumbnail nếu có); **Tab kênh chưa có nội dung**: vẫn hiển thị tab nhưng bên trong ghi "Chưa có nội dung"; **Khi campaign có Biến thể đối tượng**: bên trong mỗi trigger card hiển thị hàng tab biến thể (ví dụ: "Tất cả (dự phòng)" / "Nguy cơ rời mạng" / ...); tab mặc định = Biến thể 1 ("Tất cả (dự phòng)"); click tab biến thể → nội dung bên dưới cập nhật hiển thị nội dung của biến thể đó; tất cả chỉ đọc, không có thao tác chỉnh sửa; **Khi campaign không có biến thể** (AND mode hoặc chỉ có 1 biến thể mặc định): không hiển thị hàng tab biến thể, hiển thị thẳng nội dung |
