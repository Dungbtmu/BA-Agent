...
| **Hoạt động** | 1. Admin nhập Trigger code (bắt buộc), Tên (bắt buộc), chọn Loại (Realtime / Near Realtime / Offline), Event source (chỉ BSS hoặc OCS — không có CRM hoặc Other), Trạng thái (mặc định Active), Mô tả (optional) <br>2. Admin thêm dòng Payload: nhập Tên tham số + Mô tả + chọn Định dạng (text / date / number / boolean / currency) + chọn Nguồn (BSS / OCS — nguồn hệ thống cung cấp giá trị cho tham số đó tại runtime; có thể khác với Event source của trigger); có thể thêm nhiều dòng hoặc xóa dòng <br>3. Admin click [Lưu]: hệ thống validate → đóng modal + cập nhật bảng + toast "Đã lưu trigger ✓" <br>**[Exception — trigger code trùng]**: Inline error "Trigger code đã tồn tại" — không đóng modal <br>**[Exception — trigger code sai format]**: Inline error "Chỉ dùng chữ in hoa, số và gạch dưới (_)" |
| **Quy tắc nghiệp vụ** | - Trigger code chỉ chứa chữ in hoa, số và gạch dưới (`_`); không được trùng với code đã có trong hệ thống <br>- Event source chỉ được chọn BSS hoặc OCS — không có CRM hoặc Other; CRM là nguồn data để đọc thông tin KH (app install, đăng nhập) nhưng không phải event source được cấu hình trực tiếp trên UI <br>- Phải có ít nhất 1 dòng Payload; tên tham số và Mô tả không được để trống; Định dạng bắt buộc chọn từ danh sách: text / date (DD/MM/YYYY) / number / boolean / currency (VND) <br>- **Nguồn per param** (BSS/OCS): là nguồn hệ thống cung cấp giá trị thực cho từng tham số tại runtime — khác biệt với Event source của trigger (nguồn phát sinh event); ví dụ trigger `LOW_DATA_BALANCE` có Event source = BSS nhưng param `data_con_lai` có Nguồn = OCS vì OCS mới lưu charging data <br>- Trigger mới tạo mặc định Active; xuất hiện ngay trong dropdown trigger khi QTV tạo campaign |
...
| **Hậu điều kiện** | - Không thay đổi dữ liệu |
| **Hoạt động** | 1. Hệ thống load và hiển thị thông tin trigger chỉ đọc: Trigger code, Tên, Loại, Source (BSS hoặc OCS), Trạng thái, Mô tả <br>2. Hệ thống hiển thị bảng Payload: danh sách tên tham số + Mô tả + Định dạng tương ứng <br>3. Hệ thống hiển thị danh sách campaign đang dùng trigger này (tên campaign + trạng thái) <br>4. **Admin**: hiển thị thêm nút [Sửa] → UC-TRG-03 <br>5. Người dùng click [Đóng] → quay về danh sách |
| **Quy tắc nghiệp vụ** | - Toàn bộ nội dung chỉ đọc — không có ô nhập liệu với cả 2 role <br>- Danh sách campaign đang dùng hiển thị tất cả trạng thái (Active, Paused, Pending, Draft) <br>- Nút [Sửa] chỉ hiển thị với role Admin; QTV không thấy nút này <br>- Bảng Payload hiển thị đầy đủ 4 cột: Tên tham số, Mô tả, Định dạng, Nguồn (BSS/OCS) |
...
| STT | Tên thành phần | Định dạng | Bắt buộc | Mặc định | Mô tả |
|-----|----------------|-----------|----------|----------|-------|
| 1 | Nút [+ Thêm Trigger] | Button | – | – | Mở Modal 5B (Thêm Trigger) với tất cả trường trống |
...
| 5 | Cột Code | Text | – | – | Trigger code hiển thị in hoa |
| 6 | Cột Tên | Text | – | – | Tên đầy đủ của trigger |
| 7 | Cột Source | Text | – | – | BSS / OCS |
| 8 | Cột Trạng thái | Status chip | – | – | Active = chip xanh lá; Inactive = chip xám |
| 9 | Cột Hành động | Buttons | – | – | **[Sửa]** → mở Modal 5B pre-filled; nếu trigger đang được dùng trong ít nhất 1 campaign Active → hiển thị cảnh báo phía trên Modal: "Trigger này đang được dùng trong N campaign Active — thay đổi sẽ có hiệu lực với sự kiện tiếp theo" (không chặn, Admin vẫn sửa được); <br>**[Tắt]** → confirm dialog (2 biến thể): nếu trigger không dùng trong Active campaign nào → "Tắt trigger [CODE]?" [Hủy] / [Tắt]; nếu trigger đang dùng trong ít nhất 1 Active campaign → "Trigger [CODE] đang được dùng trong các campaign: [danh sách tên]. Tắt trigger sẽ khiến các campaign này không nhận sự kiện từ [CODE] nữa. Tiếp tục tắt?" [Hủy] / [Xác nhận Tắt]; sau khi xác nhận: nút bị khoá + đang xử lý → trạng thái chuyển Inactive; các campaign Active vẫn tiếp tục chạy nhưng trigger [CODE] không còn kích hoạt được; <br>**[Bật]** → không confirm → nút bị khoá + đang xử lý → trạng thái chuyển Active; <br>**Lưu ý**: [Tắt] và [Bật] phải bị khoá trong suốt thời gian gọi hệ thống để tránh thao tác trùng |

### 5B — Bảng đặc tả component — Modal Thêm/Sửa Trigger

| STT | Tên thành phần | Định dạng | Bắt buộc | Mặc định | Mô tả |
|-----|----------------|-----------|----------|----------|-------|
| 1 | Trigger code | Text input | Có | Trống | Tối đa 100 ký tự; chỉ chấp nhận chữ in hoa A–Z, số 0–9 và gạch dưới `_`; không được bắt đầu bằng số hoặc gạch dưới; không được kết thúc bằng gạch dưới; không được có 2 gạch dưới liên tiếp (`__`); không được trùng với trigger đã có trong hệ thống; placeholder: `"VD: SIM_ACTIVATED"`; **Thời điểm validate**: kiểm tra format ngay khi rời khỏi ô (on-blur); kiểm tra trùng code khi nhấn [Lưu]; **Thông báo lỗi**: "Trigger code không được để trống" / "Chỉ dùng chữ in hoa, số và gạch dưới (_). Không bắt đầu hoặc kết thúc bằng gạch dưới, không có gạch dưới đôi" / "Trigger code đã tồn tại — vui lòng chọn tên khác"; **Chế độ Sửa**: trường này chỉ đọc, không cho phép thay đổi code sau khi đã tạo |
| 2 | Tên trigger | Text input | Có | Trống | Tối đa 200 ký tự; cho phép mọi ký tự (tiếng Việt, tiếng Anh, ký tự đặc biệt); placeholder: `"VD: Kích hoạt SIM thành công"`; **Thời điểm validate**: on-blur; **Thông báo lỗi**: "Tên trigger không được để trống" |
| 3 | Loại | Dropdown | Có | Realtime | Options: Realtime / Near Realtime / Offline; luôn có giá trị mặc định nên không thể để trống; không cần validate riêng |
| 4 | Event source | Dropdown | Có | BSS | Options: BSS / OCS — không có CRM hoặc Other; luôn có giá trị mặc định; khi thay đổi Event source → cột Nguồn trong bảng payload (STT 7d) tự cập nhật theo cho tất cả dòng chưa được chỉnh thủ công |
| 5 | Trạng thái | Radio | Không | Active | Active / Inactive; luôn có giá trị mặc định nên không cần validate |
| 6 | Mô tả | Text area | Không | Trống | Tối đa 500 ký tự; cho phép mọi ký tự; placeholder: `"VD: Fired khi KH kích hoạt SIM mới thành công"`; không validate nội dung |
| 7 | Bảng tham số payload | Table | Có | 1 dòng trống | Bắt buộc có ít nhất 1 dòng hợp lệ khi lưu; không giới hạn số dòng tối đa; thông báo lỗi khi lưu mà bảng không có dòng nào: "Trigger phải có ít nhất 1 tham số payload"; xem chi tiết từng cột tại STT 7a–7d bên dưới |
| 7a | — Cột Tên tham số | Text input | Có | Trống | Tối đa 100 ký tự; chỉ chấp nhận chữ thường a–z, số 0–9 và gạch dưới `_` (snake_case); không được bắt đầu bằng số hoặc gạch dưới; không được kết thúc bằng gạch dưới; không được có 2 gạch dưới liên tiếp; không được trùng tên với tham số khác trong cùng trigger; placeholder: `"ten_param"`; **Thời điểm validate**: on-blur; **Thông báo lỗi**: "Tên tham số không được để trống" / "Tên tham số chỉ dùng chữ thường, số và gạch dưới. Không bắt đầu hoặc kết thúc bằng gạch dưới, không có gạch dưới đôi" / "Tên tham số đã tồn tại trong trigger này" |
| 7b | — Cột Mô tả | Text input | Có | Trống | Tối đa 200 ký tự; cho phép mọi ký tự; placeholder: `"Mô tả nội dung tham số"`; **Thời điểm validate**: on-blur; **Thông báo lỗi**: "Mô tả không được để trống" |
| 7c | — Cột Định dạng | Dropdown | Có | text | Options: text / date (DD/MM/YYYY) / number / boolean / currency (VND); luôn có giá trị mặc định nên không thể để trống |
| 7d | — Cột Nguồn | Dropdown | Có | Theo Event source | Options: BSS / OCS; mặc định lấy theo Event source của trigger (STT 4); tự động cập nhật khi Admin thay đổi Event source — chỉ áp dụng cho dòng chưa chỉnh thủ công |
| 8 | Nút [+ Thêm tham số] | Button | – | – | Thêm 1 dòng trống vào cuối bảng payload; dòng mới kế thừa giá trị Nguồn từ Event source hiện tại; Định dạng mặc định = text |
| 9 | Nút [✕] xóa dòng | Button | – | – | Xóa dòng tương ứng khỏi bảng; bị khoá khi bảng chỉ còn 1 dòng (không thể xóa dòng cuối cùng); thao tác xóa không cần confirm |
| 10 | Nút [Hủy] | Button | – | – | Đóng modal, không lưu bất kỳ thay đổi nào; không cần confirm khi hủy |
| 11 | Nút [Lưu] | Button | – | – | **Thứ tự validate khi nhấn [Lưu]**: (1) Trigger code — format + không trống + không trùng; (2) Tên trigger — không trống; (3) Bảng payload — có ít nhất 1 dòng; (4) Từng dòng payload — Tên tham số (format + không trống + không trùng) + Mô tả (không trống); **Khi có lỗi validate**: hiển thị thông báo lỗi ngay dưới trường lỗi, modal giữ nguyên không đóng, tự cuộn đến trường lỗi đầu tiên; **Khi hợp lệ**: nút bị khoá + hiển thị đang xử lý → khi hệ thống phản hồi thành công: đóng modal + cập nhật bảng danh sách + toast "Đã lưu trigger ✓"; khi hệ thống lỗi: toast "Lưu thất bại — vui lòng thử lại" + modal giữ nguyên + nút được mở khóa lại |
...
## Screen Admin: Duyệt Campaign & Cấu hình Trigger _(UC-CAM-05, UC-TRG-01)_
...
| 8 | Tab Cấu hình Trigger | Tab | – | – | Nội dung giống Screen 5 Trigger Management — hoàn toàn giống, không có sự khác biệt về logic hay UI; Admin có đầy đủ quyền thêm/sửa/bật/tắt trigger từ đây (xem Screen 5 để biết chi tiết spec từng component) |
