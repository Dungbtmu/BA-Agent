...
| **Tác nhân** | QTV Marketing, Admin Hệ thống |
| **Trigger** | Người dùng click [Xóa] trên toàn dòng, hoặc [×] trên một chip Campaign/Kênh cụ thể trong dòng |
...
| 6 | Cột Campaign | Chips | – | – | **Dòng Theo Campaign**: liệt kê chip tên campaign đang chặn số đó, tối đa 2 chip + "+N ⓘ" (hover/click "+N" → popover đầy đủ danh sách); mỗi chip có [×] riêng để gỡ đúng tổ hợp đó (xem cột 8 — Hành động) <br>**Dòng Toàn hệ thống**: hiển thị "Toàn hệ thống" (chữ đậm, badge tím), không phải chip, không có [×] per-chip |
| 7 | Cột Kênh | Chips | – | – | **Dòng Theo Campaign**: liệt kê chip kênh đang chặn số đó (Push/Zalo OA/SMS/USSD/Banner/Email), tối đa 2 chip + "+N ⓘ"; mỗi chip gắn với đúng cặp campaign-kênh, click [×] trên chip Kênh hoặc chip Campaign tương ứng đều xóa cùng 1 tổ hợp <br>**Dòng Toàn hệ thống**: hiển thị "Tất cả kênh" |
| 8 | Nút [Xóa] | Button | – | – | **Xóa toàn dòng**: confirm dialog liệt kê đầy đủ tổ hợp sẽ bị gỡ — dòng Theo Campaign → "Xóa [số] khỏi TẤT CẢ [N] campaign-kênh đang chặn: [danh sách]?"; dòng Toàn hệ thống → "Xóa [số] khỏi Blacklist toàn hệ thống?" [Hủy] / [Xóa] → nút bị khoá + đang xử lý → khi hệ thống phản hồi: xóa dòng khỏi bảng + toast "Đã xóa ✓"; nếu hệ thống lỗi: toast "Xóa thất bại — vui lòng thử lại" + nút mở khóa lại <br>**Xóa 1 chip**: click [×] trên chip Campaign/Kênh → confirm dialog "Xóa [số] khỏi blacklist campaign [Y] kênh [Z]?" [Hủy] / [Xóa] → xác nhận: gỡ đúng tổ hợp đó, dòng cập nhật còn lại các chip khác; nếu là chip cuối cùng → dòng biến mất khỏi bảng <br>**Dòng Toàn hệ thống chỉ Admin thấy nút [Xóa]**, QTV thấy dạng chỉ đọc |
...
*Phiên bản: V4.16 | Ngày: 08/09/2026 | Tác giả: Jun (BA)*
*Đợt cập nhật gần nhất: lọc khối "Điều kiện lọc theo Kênh" (Screen 3 Section 4 STT 4b) theo tab biến thể đang mở khi có ≥ 2 Audience Variant — tránh conflict hiển thị khi QTV đang soạn cho 1 phân khúc mà vẫn thấy điều kiện lọc của phân khúc khác không liên quan.*
