...
| **Hoạt động** | 1. Hệ thống tải danh sách template (mặc định 20 bản ghi/trang, **sắp xếp theo số lần dùng nhiều nhất**) <br>1a. Hiển thị bảng: Tên Template, Trigger, Kênh hỗ trợ, Số lần dùng, Hành động <br>2. QTV tùy chọn nhập từ khóa tìm kiếm theo tên template <br>2a. Hệ thống lọc realtime <br>3. QTV tùy chọn lọc theo Kênh hoặc Trạng thái (Active/Inactive) <br>3a. Hệ thống lọc bảng theo điều kiện đã chọn <br>4. QTV click "+N ⓘ" trên cột Trigger <br>4a. Hệ thống mở popover hiển thị đầy đủ danh sách trigger đã gắn cho template đó <br>5. QTV click số "X lần" ở cột Dùng <br>5a. Hệ thống hiển thị popover danh sách campaign đang dùng template đó <br>6. QTV click [Xóa] trên một template <br>6a. Hệ thống hiển thị confirm dialog "Xóa template [Tên]? Hành động này không thể hoàn tác." [Hủy] / [Xóa] <br>6b. Xác nhận → nếu template chưa được campaign nào tham chiếu (cột Dùng = 0): xóa cứng, toast "Đã xóa template ✓", biến mất khỏi danh sách <br>**[Exception — Xóa template đang được campaign dùng]**: nút [Xóa] disabled; hover → tooltip "Không thể xóa — template đang được N campaign sử dụng (kể cả Draft/Ended). Vui lòng gỡ khỏi các campaign đó trước, hoặc dùng [Tắt] để ẩn khỏi danh sách chọn mới" <br>**[Alternative]**: QTV click [+ Tạo Template] → navigate /templates/new |
| **Quy tắc nghiệp vụ** | - Cột "Dùng" hiển thị tổng số campaign có tham chiếu template này, kể cả Draft và Ended (không chỉ Active) — click → popover liệt kê tên campaign kèm trạng thái <br>- Template Inactive hiển thị grayed out trong danh sách; không xuất hiện trong dropdown khi soạn nội dung campaign <br>- Tìm kiếm áp dụng cho tên template <br>- **Cột Trigger**: hiển thị Trigger áp dụng khai báo tại UC-TPL-01/UC-TPL-03, tối đa 2 chip + "+N ⓘ" (giống cách cột Trigger tại Campaign List hiển thị); chỉ mang tính phân loại/tra cứu, không giới hạn phạm vi sử dụng template khi soạn campaign; template chưa gắn Trigger nào hiển thị "–" <br>- **Xóa template**: chỉ cho phép xóa cứng khi template chưa được bất kỳ campaign nào tham chiếu (Dùng = 0, tính cả Draft/Ended); template đã từng được dùng — dù campaign đó đã Ended — không thể xóa, chỉ có thể [Tắt] để ẩn khỏi lựa chọn mới, vì campaign cũ vẫn cần giữ nguyên nội dung đã copy tại thời điểm tạo (xem UC-TPL-03) |
...
| 6 | Cột Trigger | Chip | – | – | Danh sách Trigger áp dụng (xem UC-TPL-01/UC-TPL-03) hiển thị tối đa 2 chip; nếu template gắn nhiều hơn 2 trigger thì hiện "+N ⓘ" (N = số trigger còn lại), hover/click → popover đầy đủ (mã trigger + tên); template chưa gắn Trigger nào hiển thị "–" |
...
*Phiên bản: V4.3 | Ngày: 21/08/2026 | Tác giả: Jun (BA)*
