...
| 15/09/2026 | Jun | UC-BL-03 (Hoạt động, Quy tắc nghiệp vụ); Screen 6A (STT 6, 7, 8, 8b mới) | U | **Sửa gap phát hiện qua BA rà soát UI Blacklist Management — xóa 1 chip Kênh/Campaign trên dòng ma trận nhiều-nhiều (≥ 2 campaign VÀ ≥ 2 kênh cùng 1 số):** trước đây câu URD "click [×] trên chip Kênh hoặc chip Campaign tương ứng đều xóa cùng 1 tổ hợp" chỉ đúng khi dòng đơn giản (1 kênh × 1 campaign) — không mô tả rõ trường hợp 1 chip đại diện cho NHIỀU tổ hợp cùng lúc (ví dụ chip Kênh "Push" gộp chung Push-CampaignA và Push-CampaignB), dẫn đến rủi ro hiểu sai là xóa ngẫu nhiên 1 trong nhiều tổ hợp. Chốt: (1) **Xóa 1 chip trên ma trận nhiều-nhiều gỡ TẤT CẢ tổ hợp chứa giá trị đó** — hành vi có chủ đích (xóa hẳn 1 giá trị khỏi mọi tổ hợp), confirm dialog liệt kê rõ số lượng + danh sách tổ hợp bị ảnh hưởng; (2) **Bổ sung nút [Xem chi tiết tổ hợp]** (chỉ hiện khi dòng là ma trận nhiều-nhiều thật) — popover liệt kê từng tổ hợp cụ thể, cho phép xóa lẻ đúng 1 tổ hợp (ví dụ chỉ Push-CampaignA, giữ nguyên Push-CampaignB) mà chip đơn lẻ không làm được; (3) **Text confirm dialog phải phân biệt rõ theo 3 case xóa** (1 tổ hợp duy nhất / nhiều tổ hợp qua 1 chip gộp / xóa lẻ qua popover) — case xóa lẻ phải nêu rõ số tổ hợp còn lại sau khi xóa để tránh hiểu nhầm đây là tổ hợp duy nhất. | V4.17 |
| 08/09/2026 | Jun | Screen 3 Section 4 (STT 4b) | U | **Lọc khối "Điều kiện lọc theo Kênh" theo tab biến thể đang mở khi có ≥ 2 Audience Variant — phát hiện qua BA rà soát:** trước đây khối này luôn hiện đủ **mọi** phân khúc đã chọn ở Section 3, kể cả khi QTV đang mở đúng 1 tab biến thể đã gán riêng 1 phân khúc cụ thể — gây conflict hiển thị: QTV đang soạn cho phân khúc A vẫn thấy nguyên khối điều kiện lọc của phân khúc B không liên quan, dễ nhầm lẫn sửa sai chỗ. Chốt: có ≥ 2 biến thể và đang ở tab đã gán phân khúc cụ thể → chỉ hiện đúng khối của phân khúc đó; tab Biến thể 1 "Tất cả (dự phòng)" (không gán riêng) và trường hợp chưa bật Variant (0-1 biến thể, không có "phân khúc đang soạn" để lọc theo) đều giữ nguyên hiện tất cả khối như cũ. | V4.16 |
| 07/09/2026 | Jun | UC-CAM-05 (Hậu điều kiện, Hoạt động, Quy tắc nghiệp vụ) | U | **Bổ sung chặn trùng độ ưu tiên khi Admin Phê duyệt campaign — lỗ hổng thứ 4 phát hiện qua case BA đặt ra:** campaign A đang Active priority = 2; campaign B đang Pending priority cũng = 2 (không bị chặn lúc Gửi duyệt vì lúc đó B còn ở Pending, chưa phải Active nên không nằm trong phạm vi so trùng V4.14); Admin duyệt B → B chuyển Active → hệ thống có 2 campaign Active cùng priority mà không hề bị chặn, vì UC-CAM-05 (Duyệt/Từ chối) trước đó hoàn toàn không kiểm tra trùng. Xác nhận: thời điểm campaign thực sự "tham gia xếp hạng Active" là lúc **Phê duyệt (Pending → Active)**, không phải lúc Gửi duyệt (Draft → Pending) — đây là chỗ cần chặn, không phải chặn thêm ở Builder. **Chọn phương án (BA quyết định giữa 2 lựa chọn)**: khi Admin bấm [Duyệt] mở dialog xác nhận, hệ thống check trùng ngay lúc đó — trùng thì **dialog đổi hẳn nội dung sang cảnh báo** (không phải disable nút trên danh sách), chỉ còn [Đóng], không có nút xác nhận duyệt (không phải "bắt Admin Từ chối") — Admin không tự sửa priority tại màn Duyệt (giữ đúng ranh giới vai trò: Admin chỉ duyệt nội dung, không chỉnh cấu hình); [Từ chối] vẫn là hành động Admin tự quyết định như bình thường, chỉ là con đường duy nhất dẫn đến QTV sửa lại priority rồi gửi duyệt lại qua đúng vòng chặn trùng đã có ở Campaign Builder (UC-CAM-02, V4.14). | V4.15 |
| 25/08/2026 | Jun | UC-CAM-01 (Hoạt động, Quy tắc nghiệp vụ); UC-CAM-02 (Hoạt động, Quy tắc nghiệp vụ); UC-PRIORITY-01 (Quy tắc nghiệp vụ); Screen 3 (STT 5) | U | **Thống nhất quy tắc chặn trùng độ ưu tiên giữa 3 nơi sửa priority** — phát hiện mâu thuẫn qua rà soát theo yêu cầu BA: Priority Matrix (UC-PRIORITY-01, Screen Settings Tab 3 STT 3.5) đã chặn cứng trùng số (`hasDuplicate` → disable nút Lưu) từ trước, nhưng Campaign Builder (UC-CAM-02, Screen 3 STT 5) lại ghi "cho phép trùng, không validate", còn Campaign List (UC-CAM-01) không quy định rõ — tạo lỗ hổng: QTV có thể tạo/sửa campaign trùng priority với campaign Admin đã sắp xếp trong Priority Matrix mà không bị chặn, Admin không biết cho đến khi mở lại Priority Matrix. **Chốt quy tắc chung**: không cho phép 2 campaign Active cùng độ ưu tiên, phạm vi so trùng chỉ tính trong tập campaign đang Active (Draft/Pending/Paused/Ended không tham gia). Áp dụng: (1) **Campaign List** — nhập số trùng campaign Active khác → lỗi inline, chặn mở confirm dialog cho đến khi đổi số; Draft không so trùng; (2) **Campaign Builder** — Lưu Nháp vẫn cho lưu dù trùng, chỉ chặn tại [Gửi duyệt] (blocking issue mới); (3) **Priority Matrix** — giữ nguyên hành vi chặn cứng đã có. Tie-break theo `created_at` (đã có từ trước) giữ lại làm phòng vệ cuối cho race condition, không còn là cơ chế xử lý trùng chính. | V4.14 |
| 22/08/2026 | Jun | UC-CAM-07 (Tiền điều kiện, Hậu điều kiện, Hoạt động, Quy tắc nghiệp vụ); Quy tắc nghiệp vụ chung Khối 3 (PARAM_INVALID); Screen 2 (STT 9); Screen 2B (STT 3) | U | **Bổ sung nhánh "Mở khóa param/điều kiện lọc" cho UC-CAM-07 — case BA phát hiện qua team hỏi lại comment nghiệm thu:** campaign đang **Chờ duyệt (Pending)** → Admin **Khóa** param/điều kiện lọc đang dùng → tự động chuyển **Tạm dừng (Paused)** + gắn cờ (đúng policy đã có từ trước) → Admin **Mở khóa** lại đúng param/điều kiện lọc đó (không sửa nội dung message) → QTV bấm [Bật]. Trước đây UC-CAM-07 chỉ có 2 nhánh: (a) không có cờ → Bật thẳng Active, (b) còn cờ do Khóa → khóa vĩnh viễn nút [Bật], chỉ resume qua [Sửa] → Draft → gửi duyệt lại — **thiếu nhánh cho trường hợp cờ do Khóa nhưng param/điều kiện lọc đã được Mở khóa lại**, dễ hiểu lầm là tự động Bật thẳng Active dù trạng thái gốc trước khi Paused là Pending. Bổ sung: (1) hệ thống **lưu trạng thái gốc** (`pre_pause_status`: Active hoặc Pending) tại thời điểm tự chuyển Paused; (2) khi Admin Mở khóa lại đúng param/điều kiện lọc, nút [Bật] không còn bị disabled, bấm vào → confirm dialog → campaign quay về **đúng trạng thái gốc đã lưu** (Pending nếu gốc là Pending — **không tự động chạy thẳng Active**, vẫn phải qua Admin duyệt lại vì cấu hình trigger đã có giai đoạn không hợp lệ trong lúc dừng, cùng lý do với nhánh Sửa đã có). Đây là con đường resume **thứ 2** (nhanh hơn, không cần QTV vào [Sửa]), song song với con đường cũ (qua [Sửa] khi cần đổi nội dung message/điều kiện lọc). | V4.13 |
| 22/08/2026 | Jun | Screen 2B (STT 5, 6, 9, 10) | U | **Đồng bộ Screen 2B (Campaign Detail View) với các nghiệp vụ đã đổi qua nhiều đợt patch trước** — phát hiện qua đánh giá theo yêu cầu BA: các đợt patch trước (V4.1 Nhắc lại, V4.5 bỏ Reach) chỉ rà soát kỹ Campaign Builder, bỏ sót màn Campaign Detail View dù hiển thị cùng dữ liệu campaign. (1) **Xóa 3 chỗ còn sót Reach ước tính**: STT 5 (Section 2 — bỏ "ước tính số tin"), STT 6 (Section 3 — bỏ "số KH ước tính"), STT 10 (Section 6 — bỏ "số KH tiếp cận cuối cùng"); (2) **Bổ sung hiển thị Nhắc lại (Re-engagement)** vào STT 9 (Kênh & Lịch gửi) — trước đó hoàn toàn không có, dù đã thêm nghiệp vụ từ V4.1; (3) làm rõ STT 10 không còn khối Blackout riêng (đã dời sang STT 9 từ trước, nay ghi rõ tránh hiểu nhầm). | V4.12 |
| 22/08/2026 | Jun | Screen 6B (STT 2, 3, 6); Screen 2 (STT 7); UC-CAM-01 (Hoạt động, Quy tắc nghiệp vụ); UC-CAM-02 (Quy tắc nghiệp vụ — đơn vị %) | U | **Rà soát bổ sung validate cho các field đã update V4.0-V4.10** theo yêu cầu BA: (1) **Modal 6B (Thêm thủ công Blacklist) — Campaign/Kênh**: sửa mâu thuẫn UC-BL-01 (ghi "bắt buộc") vs Screen 6B (ghi "bỏ qua lặng lẽ, không báo lỗi") — BA xác nhận field thực sự bắt buộc, đổi hành vi sang chặn + hiển thị lỗi rõ "Vui lòng chọn ít nhất 1 chiến dịch" / "Vui lòng chọn ít nhất 1 kênh", đồng bộ Screen 6B STT 2/3/6; (2) **Cột Ưu tiên trên Campaign List (Screen 2 STT 7, UC-CAM-01)**: bổ sung range 1–9999 + thông báo lỗi cụ thể tại đúng vị trí input inline — trước đó chỉ có ở Campaign Builder (Screen 3 STT 5), Campaign List không lặp lại nên thiếu căn cứ cho Dev; (3) **Đơn vị % cho điều kiện lọc kiểu Số (UC-CAM-02)**: bổ sung liên kết rõ ràng đến quy định giới hạn 0–100 đã có sẵn tại I.3.1 Glossary — trước đó Quy tắc nghiệp vụ UC-CAM-02 không nhắc lại giới hạn này, dễ đọc sót nếu chỉ xem riêng UC-CAM-02. | V4.11 |
| 22/08/2026 | Jun | UC-BL-01, UC-BL-02 (Quy tắc nghiệp vụ); Screen 6B (STT 4) | U | **Bổ sung ghi chú nối giữa "N × M bản ghi" (data) và "gộp 1 dòng theo Số điện thoại" (UI, V4.9)** — phát hiện qua rà soát: UC-BL-01/UC-BL-02 mô tả đúng số bản ghi được tạo trong dữ liệu (N × M, không đổi), nhưng chưa trỏ sang hành vi hiển thị gộp đã bổ sung ở UC-BL-00/Screen 6A (V4.9), dễ khiến người đọc riêng lẻ 1 UC hiểu nhầm N × M bản ghi = N × M dòng trên bảng. Không phải conflict logic — 2 câu mô tả đúng 2 tầng khác nhau (lưu trữ vs hiển thị), chỉ thiếu liên kết chéo. Thêm 1 câu ghi chú ngắn vào Quy tắc nghiệp vụ UC-BL-01, UC-BL-02, và Screen 6B STT 4, trỏ rõ sang UC-BL-00/Screen 6A. | V4.10 |
| 22/08/2026 | Jun | UC-BL-00, UC-BL-03; Screen 6A | U | **Gộp bảng Blacklist Management theo Số điện thoại — bỏ dàn trải theo từng tổ hợp campaign-kênh:** trước đây mỗi tổ hợp (số × Campaign × Kênh) là 1 dòng riêng, 1 số bị chặn ở nhiều campaign/kênh sẽ lặp lại nhiều dòng, nhìn dàn trải. Theo yêu cầu BA, đổi sang **1 dòng = 1 số điện thoại** (trong đúng 1 phạm vi): cột Campaign và Kênh liệt kê dạng chip (tối đa 2 chip + "+N ⓘ", nhất quán cách hiển thị Trigger chip ở Campaign List/Template List); **bỏ hẳn cột Nguồn**. Cơ chế Xóa mở rộng 2 lớp: [×] trên từng chip để gỡ đúng 1 tổ hợp, và [Xóa] toàn dòng để gỡ mọi tổ hợp của số đó trong phạm vi đang xem (dialog liệt kê đầy đủ trước khi xác nhận). **Blacklist toàn hệ thống vẫn tách dòng riêng, không gộp chung với dòng Theo Campaign của cùng số** — khác bản chất và khác quyền thao tác. Dữ liệu lưu trữ nội bộ không đổi (vẫn theo từng tổ hợp) — chỉ đổi cách UI gộp hiển thị. Cập nhật UC-BL-00 (Hoạt động, Quy tắc nghiệp vụ), UC-BL-03 (Hoạt động, Quy tắc nghiệp vụ — tách 2 luồng xóa), Screen 6A (STT 1-8, đánh lại số sau khi bỏ cột Nguồn). | V4.9 |
| 22/08/2026 | Jun | UC-CAM-02 (Quy tắc nghiệp vụ); Screen 3 Section 4 (STT 4a, 11, 12) | U | **Preview Section 4 hiển thị đồng thời toàn bộ Audience Variant, không còn phải chuyển tab:** trước đây khi campaign có Audience Variant (biến thể theo phân khúc), preview cột phải chỉ hiển thị nội dung của biến thể đang active theo tab đang click — QTV phải click từng tab biến thể mới xem hết. Theo yêu cầu BA, đổi thành: có ≥ 2 biến thể → hiển thị **đồng thời N mockup card xếp dọc, cố định** (mỗi card 1 biến thể, kèm nhãn tên phân khúc ở đầu, tự cập nhật realtime), không cần click tab; áp dụng riêng theo từng tab kênh đang mở (số biến thể độc lập theo từng kênh). 0-1 biến thể vẫn giữ 1 khung preview như cũ. Làm rõ đồng thời: **cột trái (card soạn nội dung, STT 4a) vẫn giữ theo tab biến thể như cũ** — QTV soạn từng biến thể 1 tại 1 thời điểm, chỉ riêng cột phải (preview) đổi sang hiển thị toàn bộ cùng lúc; **SAMPLE PARAMS (STT 12) dùng chung 1 bộ giá trị mẫu cho mọi card**, không tách riêng theo biến thể. | V4.8 |
...
| **Hậu điều kiện** | - Phê duyệt (không trùng độ ưu tiên): Campaign → Active; hệ thống bắt đầu lắng nghe trigger <br>- Phê duyệt bị chặn do trùng độ ưu tiên: Campaign giữ nguyên Pending; Admin phải Từ chối để mở đường cho QTV sửa <br>- Từ chối: Campaign → Draft; hệ thống gửi push notification đến tài khoản QTV đã tạo campaign kèm lý do từ chối; QTV chỉnh sửa và gửi duyệt lại |
| **Hoạt động** | 1. Admin xem Campaign Detail View (chỉ đọc) của campaign Pending, hoặc xem tại danh sách campaign chờ duyệt <br>2. Admin kiểm tra toàn bộ nội dung: thông tin, trigger, audience, message, an toàn <br>3. Admin bấm [Duyệt] → hệ thống **kiểm tra trùng độ ưu tiên** với các campaign đang Active khác ngay khi mở dialog xác nhận: <br>3a. **Không trùng**: dialog hiển thị nội dung xác nhận bình thường "Duyệt chiến dịch [Tên campaign]? Chiến dịch sẽ chuyển sang trạng thái Đang chạy ngay" — [Hủy] / [Duyệt]; Admin xác nhận [Duyệt] → hệ thống chuyển trạng thái → Active; campaign bắt đầu vận hành <br>3b. **Trùng độ ưu tiên**: dialog đổi hẳn nội dung sang cảnh báo "Độ ưu tiên [N] của [Tên campaign] đang trùng với campaign [Tên campaign khác] đang Active. Vui lòng Từ chối để QTV quay về sửa lại độ ưu tiên trước khi gửi duyệt lại" — chỉ có nút [Đóng], không có nút xác nhận duyệt; Admin không tự sửa priority tại đây (không lẫn vai trò Admin duyệt nội dung với QTV cấu hình) <br>4. **[Từ chối]** (luôn khả dụng, không phụ thuộc việc có trùng priority hay không — Admin tự quyết định khi nào bấm): Admin nhập lý do từ chối (bắt buộc) → hệ thống chuyển campaign → Draft; gửi push notification đến QTV kèm lý do; QTV có thể sửa lại (bao gồm đổi độ ưu tiên nếu lý do từ chối là trùng số) và gửi duyệt lại — qua lại chặn trùng ở Campaign Builder (xem UC-CAM-02) trước khi vào Pending lần nữa |
| **Quy tắc nghiệp vụ** | - Chỉ Admin HT được thực hiện duyệt/từ chối — QTV không tự duyệt campaign của mình <br>- Campaign Pending không thể sửa cho đến khi Admin từ chối về Draft <br>- **Chặn trùng độ ưu tiên khi Phê duyệt** (thống nhất với UC-CAM-01, UC-CAM-02, UC-PRIORITY-01 — V4.15): thời điểm campaign thực sự tham gia xếp hạng Active là lúc Phê duyệt (Pending → Active), không phải lúc Gửi duyệt (Draft → Pending) — vì priority của campaign Pending chưa từng bị chặn trùng lúc còn Pending; do đó khi Admin bấm [Duyệt] mở dialog xác nhận, hệ thống kiểm tra lại: nếu trùng → **dialog đổi hẳn nội dung sang cảnh báo, chỉ còn nút [Đóng]**, không có nút xác nhận duyệt; Admin không tự sửa priority tại màn Duyệt (không lẫn vai trò Admin duyệt nội dung với QTV cấu hình); **[Từ chối] không bị ép buộc hay chỉ định** — vẫn là action Admin tự quyết định như bình thường, chỉ là con đường duy nhất dẫn đến QTV sửa lại priority nếu Admin muốn campaign này được Active <br>- **Background job kiểm tra `endDate`**: quét tất cả campaign ở mọi trạng thái (kể cả Pending); nếu thời điểm hiện tại > `endDate` → tự động chuyển → Ended; áp dụng cho cả Active, Pending, Paused; **campaign chọn Vô hạn không có `endDate` nên không bị job này chuyển Ended** (xem II.6.4) <br>- **Campaign được duyệt (chuyển Active) nhưng `startDate` vẫn còn ở tương lai**: hệ thống giữ nguyên trạng thái Active như bình thường (không tạo trạng thái riêng); Campaign List, Campaign Detail View và Dashboard hiển thị thêm badge phụ "⏳ Chưa tới ngày bắt đầu" cạnh status chip Active để phân biệt với campaign đang thực sự gửi tin; badge tự ẩn khi thời điểm hiện tại ≥ `startDate` (không cần thao tác thủ công); trong lúc chưa tới `startDate`, pipeline vẫn bỏ qua mọi trigger event khớp campaign này theo đúng quy tắc tại II.6.4 ("Trigger xảy ra trước startDate") |
...
| **Tiền điều kiện** | - Campaign ở trạng thái Paused <br>- **Trường hợp thường**: Campaign không còn cờ `PARAM_INVALID` và không còn cờ `FILTER_INVALID` <br>- **Trường hợp còn cờ do Khóa nhưng param/điều kiện lọc đã được Mở khóa lại**: xem nhánh riêng ở Hoạt động bước 1c — đây là tiền điều kiện đặc thù, khác 2 nhánh 1a/1b |
| **Hậu điều kiện** | - **Không có sửa param/điều kiện lọc trong lúc Paused**: Campaign chuyển → Active; hệ thống tiếp tục lắng nghe trigger <br>- **Có sửa (không phải Khóa) param/điều kiện lọc của trigger đang dùng trong lúc Paused**: Campaign chuyển → Pending (Chờ duyệt); Admin phải duyệt lại trước khi Active <br>- **Cờ do Khóa nhưng param/điều kiện lọc đã Mở khóa lại**: Campaign chuyển về đúng **trạng thái gốc trước khi tự Paused** (`pre_pause_status` — Active hoặc Pending); cờ được gỡ |
| **Hoạt động** | 1. Người dùng click [Bật] <br>1a. **Không có param/điều kiện lọc nào của trigger đang dùng bị Admin Sửa kể từ lúc campaign chuyển Paused**: hệ thống chuyển trạng thái → Active ngay (không cần confirm); toast "Campaign đã kích hoạt lại"; button chuyển từ [Bật] → [Dừng] <br>1b. **Có param hoặc điều kiện lọc của trigger đang dùng đã bị Admin Sửa (không phải Khóa) trong lúc campaign đang Paused**: hệ thống hiển thị confirm dialog "Tham số/điều kiện lọc của trigger đã được cập nhật trong lúc campaign tạm dừng. Campaign sẽ chuyển về Chờ duyệt để Admin xem xét lại trước khi chạy tiếp. Tiếp tục?" [Hủy] / [Xác nhận] → xác nhận: chuyển trạng thái → Pending; toast "Campaign đã chuyển về Chờ duyệt — cần Admin duyệt lại"; hủy: đóng dialog, campaign vẫn Paused <br>1c. **Campaign đang mang cờ PARAM_INVALID/FILTER_INVALID do Khóa, nhưng Admin đã Mở khóa lại đúng param/điều kiện lọc đó** (nút [Bật] không còn bị disabled ở bước này — xem Exception cũ chỉ áp dụng khi param/điều kiện lọc vẫn đang Khóa): hệ thống hiển thị confirm dialog "Tham số/điều kiện lọc đã được mở khóa. Campaign sẽ quay về trạng thái [Chờ duyệt/Active — điền theo `pre_pause_status`] trước khi tạm dừng. Tiếp tục?" [Hủy] / [Xác nhận] → xác nhận: gỡ cờ, chuyển trạng thái → đúng `pre_pause_status` đã lưu (Pending nếu campaign đang Pending lúc bị Khóa; Active nếu đang Active lúc bị Khóa); toast tương ứng "Campaign đã chuyển về Chờ duyệt — cần Admin duyệt lại" (nếu về Pending) hoặc "Campaign đã kích hoạt lại" (nếu về Active); hủy: đóng dialog, campaign vẫn Paused, cờ chưa gỡ <br>**[Exception — còn cờ PARAM_INVALID và param vẫn đang bị Khóa]**: Nút [Bật] disabled; hover → tooltip "Campaign đang có tham số không hợp lệ do trigger đã thay đổi — vui lòng vào [Sửa] để cập nhật nội dung message trước khi gửi duyệt lại"; QTV chỉ còn lựa chọn [Sửa], không có đường tắt bật thẳng <br>**[Exception — còn cờ FILTER_INVALID và điều kiện lọc vẫn đang bị Khóa]**: Nút [Bật] disabled; hover → tooltip "Campaign đang có điều kiện lọc không hợp lệ do trigger đã thay đổi thuộc tính lọc — vui lòng vào [Sửa] để cập nhật điều kiện lọc trước khi gửi duyệt lại"; xử lý giống PARAM_INVALID — chỉ resume qua [Sửa] → Draft → gửi duyệt lại (áp dụng khi param/điều kiện lọc **vẫn còn Khóa**; nếu đã Mở khóa lại → xem nhánh 1c) |
| **Quy tắc nghiệp vụ** | - Kích hoạt lại không cần duyệt lại, không cần confirm dialog (ngược với Kill Switch) — áp dụng khi campaign Paused không có cờ lỗi nào **và không có param/điều kiện lọc nào của trigger đang dùng bị Sửa (không phải Khóa) trong lúc Paused** <br>- **Phân biệt với policy PARAM_INVALID/FILTER_INVALID (Khối 3)**: đây là 2 cơ chế độc lập, không thay thế nhau. Khi Admin **Khóa** param/điều kiện lọc mà campaign đang tham chiếu trực tiếp → gắn cờ, khóa vĩnh viễn nút [Bật] **trong khi param/điều kiện lọc còn đang Khóa** — chỉ resume qua [Sửa] → Draft → gửi duyệt lại (không có đường "Bật → Pending") **hoặc** qua nhánh Mở khóa dưới đây nếu Admin mở lại đúng param/điều kiện lọc đó mà không cần QTV sửa nội dung. Khi Admin **Sửa** (không Khóa) param/điều kiện lọc của trigger mà campaign Paused đang dùng — kể cả khi nội dung message/điều kiện lọc hiện tại của campaign không tham chiếu trực tiếp phần vừa sửa (nên không đủ điều kiện gắn cờ PARAM_INVALID/FILTER_INVALID) — hệ thống vẫn coi đây là "cấu hình trigger đã đổi trong lúc campaign dừng": Bật lại phải quay về Chờ duyệt để Admin xác nhận lại, tránh trường hợp Admin duyệt dựa trên cấu hình trigger cũ mà không biết trigger đã đổi <br>- Muốn sửa nội dung campaign Paused: QTV dùng nút [Sửa] → Campaign Builder → chỉnh sửa → [Lưu nháp] → campaign chuyển về Draft → gửi duyệt lại theo flow bình thường (qua Admin duyệt, không kích hoạt thẳng) <br>- Campaign Paused do policy `PARAM_INVALID`/`FILTER_INVALID`, **param/điều kiện lọc vẫn đang Khóa**: [Bật] bị khóa vĩnh viễn cho đến khi campaign được sửa qua [Sửa] → Draft → gửi duyệt lại → Admin duyệt → Active; đây là con đường resume khi QTV cần thay nội dung message/điều kiện lọc để không còn phụ thuộc param/điều kiện lọc đã Khóa <br>- **Campaign Paused do policy `PARAM_INVALID`/`FILTER_INVALID`, Admin đã Mở khóa lại đúng param/điều kiện lọc đó (không đổi nội dung message)**: đây là con đường resume **thứ 2**, nhanh hơn — không cần QTV vào [Sửa]; nút [Bật] hoạt động lại (đã hết Exception), bấm [Bật] → confirm dialog → campaign quay về đúng trạng thái gốc trước khi tự Paused (`pre_pause_status`: Pending nếu campaign đang Chờ duyệt lúc bị Khóa, Active nếu đang chạy lúc bị Khóa) — **không tự động chạy thẳng Active nếu gốc là Pending**, phải qua Admin duyệt lại vì cấu hình trigger đã có giai đoạn không hợp lệ trong lúc dừng, tương tự lý do ở nhánh Sửa (1b) |
...
- **Lưu trạng thái gốc khi tự chuyển Paused**: tại thời điểm hệ thống tự động chuyển campaign sang Paused do cờ (từ Active hoặc từ Pending), hệ thống ghi nhớ trạng thái gốc (`pre_pause_status` = Active hoặc Pending) để dùng khi resume qua nhánh Mở khóa (xem UC-CAM-07)
- **Mở khóa param đang bị cờ chặn (khác với Sửa nội dung qua UC-CAM-03)**: Admin **Mở khóa** param — bản thân hành động Mở khóa không tự xóa cờ `PARAM_INVALID` của các campaign đang bị chặn bởi param đó (đúng theo dòng "Mở khóa không kích hoạt cờ" ở trên — nó cũng không tự động gỡ cờ cũ); cờ chỉ được gỡ khi QTV bấm [Bật] trên campaign đó và param đã ở trạng thái Mở khóa hợp lệ — xem nhánh mới trong UC-CAM-07 (khác nhánh "còn cờ do Khóa" đã có, đây là nhánh "cờ do Khóa nhưng param đã Mở khóa lại")
...
| **Hoạt động** | 1. Hệ thống tải và hiển thị bảng, **gộp theo Số điện thoại** — mỗi số chỉ hiện **1 dòng duy nhất** trong mỗi phạm vi (Theo Campaign hoặc Toàn hệ thống), mặc định 20 số/trang, **sắp xếp theo ngày thêm gần nhất trong số đó**: Số điện thoại, Campaign (chip — liệt kê mọi campaign đang chặn số đó, tối đa 2 chip + "+N ⓘ"; dòng Toàn hệ thống hiển thị "Toàn hệ thống" thay vì chip), Kênh (chip tương tự — mọi kênh đang chặn số đó; dòng Toàn hệ thống hiển thị "Tất cả kênh"), Hành động ([Xóa] toàn bộ + [×] trên từng chip) <br>2. Người dùng tùy chọn filter theo Campaign, Kênh, hoặc Phạm vi (Theo Campaign / Toàn hệ thống) <br>2a. Hệ thống lọc bảng realtime theo điều kiện đã chọn — filter theo Campaign/Kênh chỉ ẩn/hiện dòng có tối thiểu 1 chip khớp điều kiện, không tách chip ra khỏi dòng <br>3. **QTV**: thấy nút [+ Thêm thủ công] và [Upload CSV] (chỉ tạo bản ghi theo Campaign) → UC-BL-01, UC-BL-02 <br>4. **Admin**: thấy thêm nút [+ Thêm vào Blacklist toàn hệ thống] mở modal có 2 tab con (Thêm thủ công / Upload CSV) → UC-BL-04, UC-BL-05 |
| **Quy tắc nghiệp vụ** | - Blacklist CVM có **2 phạm vi**: (1) **Theo Campaign + Kênh** (mặc định, QTV/Admin đều thao tác được) — số bị chặn chỉ ở đúng cặp campaign-kênh cụ thể; (2) **Toàn hệ thống** (chỉ Admin thao tác) — số bị chặn ở **tất cả campaign, tất cả kênh**, không cần chọn campaign/kênh cụ thể <br>- **Gộp hiển thị theo Số điện thoại** (không dàn trải theo từng tổ hợp): dữ liệu lưu trữ nội bộ vẫn theo từng tổ hợp (số × Campaign × Kênh) như trước, nhưng UI gộp mọi tổ hợp cùng 1 số vào **1 dòng** trong bảng, liệt kê campaign/kênh dạng chip; **Blacklist toàn hệ thống luôn tách dòng riêng, không gộp chung với dòng Theo Campaign của cùng số** — dù 1 số vừa nằm trong Blacklist toàn hệ thống vừa nằm trong Blacklist riêng của vài campaign thì vẫn hiển thị 2 dòng độc lập (khác bản chất và khác quyền thao tác) <br>- **Bỏ cột Nguồn** khỏi bảng chính — không còn hiển thị cách số được thêm vào (thủ công/upload/từ campaign); thông tin này không còn cần thiết khi đã gộp theo số <br>- Số điện thoại có thể xuất hiện trong nhiều campaign-kênh khác nhau (mỗi cặp là 1 tổ hợp lưu trữ riêng, nhưng gộp hiển thị chung 1 dòng), đồng thời có thể vừa nằm trong Blacklist toàn hệ thống vừa nằm trong Blacklist của một campaign cụ thể — 2 phạm vi độc lập, không loại trừ nhau <br>- **Đồng bộ 2 chiều**: số được chọn qua option "Chọn từ danh sách thuê bao theo kênh" trong Campaign Builder → tự xuất hiện tại đây, gộp vào dòng của số đó (thêm 1 chip campaign-kênh mới); xóa 1 chip từ màn hình Blacklist Management → cảnh báo "Số này đang dùng trong campaign [X] kênh [Y]. Xóa khỏi đây sẽ không ảnh hưởng cấu hình trong campaign." → xác nhận thì xóa chip đó khỏi Blacklist Management (không xóa khỏi campaign) |
...
| **Quy tắc nghiệp vụ** | - Số hợp lệ: 10 chữ số, bắt đầu bằng 0 <br>- Số sai định dạng bị bỏ qua, không block toàn bộ thao tác; hiển thị số lượng bị bỏ qua trong toast <br>- **Chọn nhiều Campaign + nhiều Kênh**: mỗi số được thêm tạo thành N × M bản ghi (N = số Campaign đã chọn, M = số Kênh đã chọn) — ví dụ 1 số × 2 campaign × 3 kênh = 6 bản ghi độc lập; **N × M bản ghi này vẫn hiển thị gộp chung 1 dòng theo Số điện thoại tại Blacklist Management** (xem UC-BL-00, Screen 6A) — N×M là số bản ghi lưu trữ, không phải số dòng hiển thị trên bảng <br>- Tổ hợp (số, Campaign, Kênh) đã có sẵn trong blacklist: bỏ qua (không báo lỗi riêng cho từng tổ hợp, không tạo bản ghi trùng) <br>- **Trường hợp 0 bản ghi được lưu mới** (toàn bộ tổ hợp hợp lệ đều trùng): không tính là thao tác thành công — không đóng modal, không hiện toast "Đã thêm 0"; thay vào đó hiển thị lỗi ngay trong modal để QTV biết rõ lý do và có thể sửa lại |
...
| **Quy tắc nghiệp vụ** | - **Format file CSV**: 1 cột duy nhất tên `so_dien_thoai`; có hoặc không có header đều được; encoding UTF-8; mỗi dòng 1 số điện thoại <br>- **Format số điện thoại hợp lệ**: 10 chữ số, bắt đầu bằng `0`; không chứa dấu cách, dấu gạch ngang hoặc mã quốc gia (+84); ví dụ hợp lệ: `0901234567`; ví dụ sai: `+84901234567`, `090-123-4567`, `901234567` <br>- Tối đa 100.000 dòng/lần upload (tính theo số dòng trong file, không nhân theo Campaign/Kênh) <br>- Số sai định dạng bỏ qua, không block upload; hiển thị rõ số lượng bị bỏ qua trong preview <br>- **Chọn nhiều Campaign + nhiều Kênh**: mỗi số hợp lệ trong file tạo N × M bản ghi (N = số Campaign đã chọn, M = số Kênh đã chọn); **N × M bản ghi này vẫn hiển thị gộp chung 1 dòng theo Số điện thoại tại Blacklist Management** (xem UC-BL-00, Screen 6A) <br>- Tổ hợp (số, Campaign, Kênh) đã có sẵn trong blacklist: bỏ qua, không tạo bản ghi trùng <br>- File mẫu tải về có sẵn trong modal — Dev tạo file tĩnh `blacklist_mau.csv` gồm header + 3 dòng ví dụ |
...
| **Mục tiêu** | Cho phép người dùng gỡ chặn một số điện thoại khỏi blacklist của một hoặc nhiều campaign/kênh cụ thể, hoặc khỏi Blacklist toàn hệ thống |
| **Tác nhân** | QTV Marketing, Admin Hệ thống |
| **Trigger** | Người dùng click [Xóa] trên toàn dòng, [×] trên một chip Campaign/Kênh cụ thể trong dòng, hoặc [×] trên 1 dòng trong popover "Xem chi tiết tổ hợp" |
| **Tiền điều kiện** | - Người dùng đã đăng nhập <br>- Bản ghi blacklist tồn tại |
| **Hậu điều kiện** | - Số điện thoại được gỡ khỏi 1 hoặc nhiều tổ hợp campaign-kênh tương ứng (hoặc khỏi Blacklist toàn hệ thống); áp dụng ngay cho các event tiếp theo |
| **Hoạt động** | **Xóa 1 chip — chỉ đại diện đúng 1 tổ hợp** (dòng chỉ có 1 campaign hoặc 1 kênh ở phía đối diện): <br>1a. Người dùng click [×] trên chip Campaign hoặc Kênh <br>1b. Hệ thống hiển thị confirm dialog: "Xóa số [X] khỏi blacklist campaign [Y] kênh [Z]? Số này sẽ có thể nhận tin từ campaign này." <br>1c. Xác nhận → hệ thống xóa đúng tổ hợp đó, dòng cập nhật lại chip còn lại + toast "Đã xóa ✓"; nếu đó là chip cuối cùng của dòng → dòng biến mất khỏi bảng <br>**Xóa 1 chip — đại diện NHIỀU tổ hợp (ma trận nhiều-nhiều: dòng có ≥ 2 campaign VÀ ≥ 2 kênh)**: <br>1d. Người dùng click [×] trên 1 chip Campaign hoặc Kênh — chip này đang gộp chung mọi tổ hợp chứa giá trị đó (ví dụ chip Kênh "Push" gộp cả Push-CampaignA và Push-CampaignB nếu cả hai tồn tại) <br>1e. Hệ thống hiển thị confirm dialog liệt kê đầy đủ các tổ hợp sẽ bị gỡ: "Xóa {chiến dịch/kênh} [Giá trị] khỏi danh sách chặn của [SĐT]? Thao tác này gỡ [N] tổ hợp: [Campaign1 · Kênh1], [Campaign2 · Kênh2]... Số này sẽ có thể nhận tin từ các chiến dịch/kênh trên." <br>1f. Xác nhận → hệ thống xóa toàn bộ N tổ hợp chứa giá trị đó, dòng cập nhật lại chip còn lại + toast "Đã xóa N bản ghi ✓"; nếu không còn tổ hợp nào → dòng biến mất khỏi bảng <br>**Xem và xóa lẻ 1 tổ hợp cụ thể qua popover "Xem chi tiết tổ hợp"** (chỉ hiển thị nút này khi dòng là ma trận nhiều-nhiều thật — ≥ 2 campaign VÀ ≥ 2 kênh; mục đích: chip đơn lẻ không đủ khả năng diễn đạt "chỉ xóa đúng 1 cặp Campaign-Kênh cụ thể" vì 1 chip luôn đại diện cho mọi tổ hợp chứa giá trị đó): <br>1g. Người dùng click nút [Xem chi tiết tổ hợp] cạnh nút [Xóa] toàn dòng → popover mở ra liệt kê đầy đủ từng tổ hợp (Campaign · Kênh) của số đó, mỗi dòng có [×] riêng <br>1h. Người dùng click [×] trên đúng 1 dòng tổ hợp muốn xóa → hệ thống hiển thị confirm dialog: "Xóa số [X] khỏi blacklist campaign [Y] kênh [Z]? Số này vẫn còn bị chặn ở [N-1] tổ hợp khác. Số này sẽ có thể nhận tin từ campaign [Y] qua kênh [Z]." <br>1i. Xác nhận → hệ thống xóa đúng tổ hợp đó, popover cập nhật danh sách còn lại + toast "Đã xóa ✓"; các tổ hợp khác của số đó không bị ảnh hưởng; nếu dòng không còn là ma trận nhiều-nhiều sau khi xóa (chỉ còn 1 campaign hoặc 1 kênh) → ẩn nút [Xem chi tiết tổ hợp], quay về hiển thị chip thường <br>**Xóa toàn dòng (gỡ mọi tổ hợp của số đó trong phạm vi đang xem)**: <br>2a. Người dùng click [Xóa] ở cuối dòng <br>2b. Hệ thống hiển thị confirm dialog liệt kê đầy đủ campaign-kênh sẽ bị gỡ: "Xóa số [X] khỏi TẤT CẢ [N] campaign-kênh đang chặn: [danh sách]? Số này sẽ có thể nhận tin từ toàn bộ các campaign trên."; dòng Blacklist toàn hệ thống → "Xóa số [X] khỏi Blacklist toàn hệ thống? Số này sẽ có thể nhận tin từ mọi campaign (trừ khi vẫn còn trong blacklist riêng của campaign nào đó)." <br>2c. Xác nhận → hệ thống xóa toàn bộ tổ hợp của số đó trong đúng phạm vi (Theo Campaign hoặc Toàn hệ thống) đang thao tác — không ảnh hưởng phạm vi còn lại; dòng biến mất khỏi bảng + toast "Đã xóa ✓" <br>3. Người dùng hủy ở bất kỳ dialog nào → đóng dialog, không thay đổi |
| **Quy tắc nghiệp vụ** | - **Xóa 1 chip khi chỉ đại diện 1 tổ hợp**: chỉ gỡ đúng tổ hợp campaign-kênh đó; các tổ hợp còn lại của số đó không bị ảnh hưởng <br>- **Xóa 1 chip khi đại diện nhiều tổ hợp (V4.17)**: 1 chip Campaign hoặc Kênh trên dòng ma trận nhiều-nhiều luôn gộp chung MỌI tổ hợp chứa giá trị đó — click [×] gỡ TẤT CẢ tổ hợp đó cùng lúc, không phải 1 tổ hợp ngẫu nhiên; đây là hành vi có chủ đích (xóa 1 giá trị = loại bỏ hẳn giá trị đó khỏi mọi tổ hợp của số), không phải giới hạn kỹ thuật <br>- **Xóa lẻ 1 tổ hợp cụ thể trong ma trận nhiều-nhiều (V4.17)**: chỉ thực hiện được qua popover "Xem chi tiết tổ hợp" — nút này CHỈ hiển thị khi dòng có đồng thời ≥ 2 campaign VÀ ≥ 2 kênh (dòng chỉ có 1 campaign hoặc 1 kênh thì mọi chip đã đại diện đúng 1 tổ hợp, không cần popover) <br>- **Text confirm dialog phải phân biệt rõ theo từng case xóa** — không dùng chung 1 mẫu câu cho "xóa 1 tổ hợp duy nhất", "xóa nhiều tổ hợp qua 1 chip gộp" và "xóa lẻ 1 trong nhiều tổ hợp qua popover": trường hợp xóa qua popover phải nêu rõ số lượng tổ hợp còn lại sau khi xóa (ví dụ "còn bị chặn ở N-1 tổ hợp khác") để người dùng không hiểu nhầm đây là tổ hợp duy nhất của số đó <br>- Xóa toàn dòng chỉ gỡ trong đúng phạm vi đang thao tác (Theo Campaign hoặc Toàn hệ thống) — 2 phạm vi độc lập, xóa dòng Theo Campaign không đụng đến dòng Toàn hệ thống của cùng số và ngược lại <br>- Chip nguồn "Chọn trong campaign [MÃ], kênh [X]" cũng xóa được như chip thường; sau khi xóa, tổ hợp đó không còn trong Blacklist Management nhưng cấu hình trong campaign không thay đổi (campaign vẫn giữ số đó trong danh sách blacklist nội bộ) <br>- Blacklist CVM check sau khi đã check DNC toàn hệ thống — xóa khỏi blacklist không xóa khỏi DNC BSS |
...
| **Quy tắc nghiệp vụ** | - Priority score là số nguyên dương; số càng nhỏ ưu tiên càng cao (1 = cao nhất) <br>- Chỉ campaign Active mới xuất hiện trong danh sách; Draft/Pending/Paused/Ended không tham gia xếp hạng <br>- **Không cho phép 2 campaign Active cùng priority score** (thống nhất với Campaign List — UC-CAM-01, và Campaign Builder — UC-CAM-02): nút [Lưu thứ tự] disable khi còn số trùng trong danh sách; Admin phải sửa hết trùng trước khi lưu được <br>- Tiebreak (chỉ là phòng vệ cuối cho race condition — ví dụ 2 nơi cùng lưu đồng thời trước khi kịp phát hiện trùng, về nguyên tắc không nên xảy ra vì đã chặn trùng ở cả 3 nơi sửa priority): nếu vẫn phát sinh 2 campaign cùng priority score, campaign có `created_at` sớm hơn được chọn khi xử lý event <br>- Thay đổi priority áp dụng cho event xảy ra **sau** thời điểm lưu; event đang trong queue xử lý không bị ảnh hưởng <br>- Admin không thể xóa campaign khỏi danh sách tại màn hình này — chỉ sắp xếp lại thứ tự <br>- **Sửa priority tại Priority Matrix không yêu cầu campaign chuyển về Chờ duyệt** — Admin là người duyệt campaign nên tự sắp xếp lại thứ tự không cần "tự duyệt lại chính mình"; áp dụng ngay sau khi Lưu. **Khác với** sửa priority trực tiếp trên Campaign List (UC-CAM-01) — nơi cả QTV lẫn Admin đều thao tác được: trường hợp đó luôn chuyển campaign về Pending để đảm bảo có một bước xác nhận độc lập, kể cả khi người sửa là Admin |
...
| 7 | Cột ƯU TIÊN | Number (inline edit) | – | – | Hiển thị số độ ưu tiên hiện tại; **campaign Active**: click vào ô → chuyển sang input số inline, Enter/blur để xác nhận → confirm dialog → xác nhận thì campaign chuyển Pending (xem UC-CAM-01 bước 5, quy tắc nghiệp vụ); **campaign Draft**: click vào ô → input số inline, Enter/blur lưu ngay — **không có confirm dialog, không đổi trạng thái**; campaign Pending/Paused/Ended: hiển thị dạng text tĩnh, không click được; **Validate** (áp dụng cả Active và Draft, cùng ngưỡng với Screen 3 STT 5): số nguyên dương từ 1 đến 9999, không cho để trống; nhập số âm/0/thập phân/ngoài khoảng/để trống → input viền đỏ, lỗi inline ngay dưới ô "Độ ưu tiên phải là số nguyên từ 1 đến 9999", không lưu, giữ nguyên giá trị cũ cho đến khi QTV sửa lại hoặc Esc để hủy |
...
| 5 | Section 2 — Trigger & Logic | Read-only | – | – | Chế độ (Basic/Advanced), logic (OR/AND), danh sách trigger kèm số thứ tự ưu tiên, quy tắc khi khớp nhiều trigger; **không hiển thị số liệu ước tính** (Reach ước tính đã bỏ hoàn toàn khỏi campaign từ V4.5 — xem I.3.1 Glossary mục Reach) |
| 6 | Section 3 — Audience | Read-only | – | – | Danh sách phân khúc đã chọn (tag), logic phân khúc; **không hiển thị số lượng KH ước tính** (Reach ước tính đã bỏ hoàn toàn khỏi campaign từ V4.5) và **không hiển thị điều kiện lọc tại đây** — điều kiện lọc con của từng phân khúc giờ khác nhau theo từng kênh, xem chi tiết tại Section 4 (STT 7) theo đúng tab kênh tương ứng |
...
| 9 | Kênh & Lịch gửi | Read-only | – | – | Hiển thị lịch chung hoặc lịch riêng từng kênh; mỗi kênh ghi rõ thời gian gửi + giờ giới nghiêm (Bật/Tắt + khung giờ + cách xử lý: Hủy luôn / Hoãn đến đầu khung giờ); **Nhắc lại (Re-engagement, xem II.6.10)**: hiển thị "Cho phép nhắc lại: Bật/Tắt"; nếu Bật → hiển thị thêm "Số lần nhắc lại tối đa: [N]" và "Khoảng cách tối thiểu: [N] ngày" |
| 10 | Section 6 — An toàn | Read-only | – | – | DNC: Bật/Tắt; Blacklist: hình thức áp dụng (Không dùng / Chọn từ danh sách / Upload tệp) + số KH; Whitelist: tương tự; **không hiển thị "Reach cuối cùng"** (đã bỏ khỏi Campaign Builder Section 6 từ V4.5, đồng bộ tại đây); không có khối Blackout riêng tại đây — giờ giới nghiêm xem tại STT 9 (Kênh & Lịch gửi) |
...
| 4a | Tên phân khúc (cột trái — card soạn nội dung) | Label | – | – | **Chỉ hiển thị khi Section 3 đã chọn ≥ 1 phân khúc**: mỗi card nội dung trigger (STT 5) hiển thị nhãn tên phân khúc đang soạn ở đầu card (ví dụ "Phân khúc: ARPU cao"); khi campaign đang dùng Audience Variant (STT 13), QTV vẫn soạn nội dung **theo từng tab biến thể** ở cột trái (chỉ soạn 1 biến thể tại 1 thời điểm, đổi tab để soạn biến thể khác) — nhãn tên phân khúc đổi theo tab biến thể đang mở; mục đích: QTV xác nhận đang soạn nội dung cho đúng phân khúc nào. **Khác với preview cột phải (STT 11)**: cột phải không còn theo tab — hiển thị đồng thời toàn bộ N khung preview của mọi biến thể, không cần chuyển tab mới xem được |
| 4b | Điều kiện lọc theo Kênh | Accordion trong từng card trigger | Không | Thu gọn | **Điều kiện lọc con của phân khúc — di chuyển từ Section 3 xuống đây** (xem ghi chú Section 3 STT 5); mỗi card nội dung trigger (STT 5) trong mỗi tab kênh có accordion [+ Thêm lọc] / [Sửa] riêng theo đúng cặp **Trigger × Phân khúc × Kênh** đang mở; mở → hiện các dòng điều kiện: [Thuộc tính ▾] [Toán tử ▾] [Giá trị] [×] — cùng cơ chế thuộc tính động theo trigger, toán tử khai báo thẳng theo thuộc tính, giá trị render theo kiểu, validate BETWEEN "đến" ≥ "từ" (blocking issue), thuộc tính bị Khóa không xuất hiện trong dropdown — **giống hệt cơ chế điều kiện lọc trước đây ở Section 3**, chỉ khác phạm vi áp dụng: mỗi kênh có bộ điều kiện riêng thay vì 1 bộ dùng chung cho mọi kênh <br>**Khởi tạo khi thêm kênh mới** (`[+ Kênh]` tại STT 3): với mỗi phân khúc đã có ở Section 3, hệ thống **tự động copy** bộ điều kiện lọc mặc định của phân khúc đó (nếu phân khúc đã có điều kiện lọc từ kênh khác đã cấu hình trước, hoặc rỗng nếu đây là kênh đầu tiên) sang kênh vừa thêm; QTV có thể sửa riêng cho từng kênh mà không ảnh hưởng kênh khác — không bắt nhập lại từ đầu <br>**Chưa chọn trigger nào ở Section 2** hoặc **trigger đã chọn chưa khai báo điều kiện lọc**: accordion bị vô hiệu hóa, hiển thị text mờ tương ứng (giống hành vi cũ ở Section 3), ẩn nút [+ Thêm lọc] <br>**Lọc theo tab biến thể đang mở khi có ≥ 2 Audience Variant** (V4.16 — tránh conflict hiển thị): mặc định khối "Điều kiện lọc theo Kênh" hiện đủ khối của **mọi phân khúc** đã chọn ở Section 3; nhưng khi campaign đã bật Audience Variant (≥ 2 biến thể) và QTV đang mở tab 1 biến thể đã gán phân khúc cụ thể → chỉ hiện **đúng 1 khối** của phân khúc đó, ẩn khối các phân khúc khác không liên quan đến biến thể đang soạn (tránh QTV nhầm lẫn sửa nhầm điều kiện của phân khúc khác trong lúc đang tập trung vào 1 biến thể); riêng tab Biến thể 1 "Tất cả (dự phòng)" (không gán phân khúc riêng) vẫn hiện đủ tất cả khối, vì đây là nội dung dự phòng cho toàn bộ audience; **0-1 biến thể** (chưa bật Variant): không có "phân khúc đang soạn" cụ thể để lọc theo — giữ hiện tất cả khối như hành vi gốc <br>Nhiều điều kiện trong cùng card quan hệ AND; [Áp dụng] cập nhật số KH sau lọc hiển thị ngay trong card |
...
| 11 | Preview realtime (cột phải ~45%) | Mockup card / danh sách mockup card | – | – | **Không có Audience Variant hoặc chỉ có 1 biến thể mặc định**: hiển thị 1 mockup card duy nhất, tự động cập nhật khi QTV gõ — không cần nhấn nút; tham số động hiển thị nguyên dạng `{{ten_bien}}` trong preview <br>**Có từ 2 Audience Variant trở lên (STT 13)**: hiển thị **đồng thời N mockup card xếp dọc, cố định — không còn dạng tab phải click từng biến thể mới xem được**; mỗi mockup card có nhãn tên phân khúc ở đầu (ví dụ "Biến thể 1 · Tất cả (dự phòng)", "Biến thể 2 · ARPU cao"); mỗi card tự động cập nhật realtime theo đúng nội dung QTV đang soạn cho biến thể đó; nội dung dài hơn khung nhìn → cuộn dọc toàn cột phải để xem hết N card; **áp dụng riêng theo từng tab kênh đang mở** — chuyển sang tab kênh khác thì hiển thị lại đúng N card theo số biến thể của kênh đó (số biến thể độc lập theo từng kênh, xem STT 13) |
| 12 | SAMPLE PARAMS | Input fields | Không | Trống | Nhập giá trị mẫu cho từng tham số; danh sách tham số = union payload của tất cả trigger đã chọn ở S2; nếu S2 chưa chọn trigger → không hiển thị field nào, chỉ ghi "Chọn trigger để xem tham số động"; giá trị đã nhập được lưu cùng draft (không mất khi tải lại trang); thay thế `{{ten_bien}}` bằng giá trị mẫu trong preview realtime; **dùng chung 1 bộ giá trị mẫu cho toàn bộ campaign** — khi có nhiều mockup card theo biến thể (STT 11), tất cả các card đều thay thế tham số bằng cùng 1 bộ giá trị này; không có SAMPLE PARAMS riêng theo từng biến thể |
...
| STT | Tên thành phần | Định dạng | Bắt buộc | Mặc định | Mô tả |
|-----|----------------|-----------|----------|----------|-------|
| 1 | Ô tìm kiếm | Search | Không | Trống | Placeholder: `"Tìm số điện thoại hoặc campaign..."`; tìm theo số điện thoại (prefix match) hoặc theo tên campaign xuất hiện trong bất kỳ chip nào của dòng (contains); không validate format khi tìm; reset về trang 1 khi thay đổi từ khóa |
| 2 | Filter Phạm vi | Listbox | Không | Tất cả | Options: Tất cả / Theo Campaign / Toàn hệ thống; reset về trang 1 khi thay đổi |
| 3 | Filter Campaign | Listbox | Không | Tất cả | Searchable — tìm theo tên hoặc mã campaign; options: Tất cả + danh sách campaign đã có trong hệ thống; disabled khi Filter Phạm vi = Toàn hệ thống; chỉ hiện các dòng có ít nhất 1 chip Campaign khớp — không tách chip khỏi dòng; **Trạng thái đang tải**: hiển thị "Đang tải..." trong dropdown khi đang lấy danh sách campaign; **Trạng thái rỗng**: "Không có campaign nào trong hệ thống"; reset về trang 1 khi thay đổi |
| 4 | Filter Kênh | Listbox | Không | Tất cả | Options: Tất cả / Push / Zalo OA / SMS / USSD / Banner / Email; chỉ hiện các dòng có ít nhất 1 chip Kênh khớp — không tách chip khỏi dòng; reset về trang 1 khi thay đổi |
| 5 | Cột Số điện thoại | Text | – | – | Hiển thị nguyên 10 chữ số, không dấu phân cách (ví dụ: 0901234567); **1 dòng = 1 số** trong đúng 1 phạm vi (không dàn trải theo từng tổ hợp campaign-kênh) |
| 6 | Cột Campaign | Chips | – | – | **Dòng Theo Campaign**: liệt kê chip tên campaign đang chặn số đó, tối đa 2 chip + "+N ⓘ" (hover/click "+N" → popover đầy đủ danh sách); mỗi chip có [×] riêng — **click [×] gỡ TẤT CẢ tổ hợp đang chứa campaign đó** (nếu campaign này đang gắn với nhiều kênh khác nhau thì gỡ hết các tổ hợp đó cùng lúc, xem cột 8 — Hành động và V4.17) <br>**Dòng Toàn hệ thống**: hiển thị "Toàn hệ thống" (chữ đậm, badge tím), không phải chip, không có [×] per-chip |
| 7 | Cột Kênh | Chips | – | – | **Dòng Theo Campaign**: liệt kê chip kênh đang chặn số đó (Push/Zalo OA/SMS/USSD/Banner/Email), tối đa 2 chip + "+N ⓘ"; **click [×] trên chip Kênh gỡ TẤT CẢ tổ hợp đang chứa kênh đó** (nếu kênh này đang gắn với nhiều campaign khác nhau thì gỡ hết các tổ hợp đó cùng lúc — khác V4.9 trở về trước, xem V4.17) <br>**Dòng Toàn hệ thống**: hiển thị "Tất cả kênh" |
| 8 | Nút [Xóa] | Button | – | – | **Xóa toàn dòng**: confirm dialog liệt kê đầy đủ tổ hợp sẽ bị gỡ — dòng Theo Campaign → "Xóa [số] khỏi TẤT CẢ [N] campaign-kênh đang chặn: [danh sách]?"; dòng Toàn hệ thống → "Xóa [số] khỏi Blacklist toàn hệ thống?" [Hủy] / [Xóa] → nút bị khoá + đang xử lý → khi hệ thống phản hồi: xóa dòng khỏi bảng + toast "Đã xóa ✓"; nếu hệ thống lỗi: toast "Xóa thất bại — vui lòng thử lại" + nút mở khóa lại <br>**Xóa 1 chip — chỉ đại diện đúng 1 tổ hợp** (dòng chỉ có 1 campaign hoặc 1 kênh): click [×] → confirm dialog "Xóa [số] khỏi blacklist campaign [Y] kênh [Z]?" [Hủy] / [Xóa] → xác nhận: gỡ đúng tổ hợp đó, dòng cập nhật còn lại các chip khác; nếu là chip cuối cùng → dòng biến mất khỏi bảng <br>**Xóa 1 chip — đại diện NHIỀU tổ hợp (ma trận nhiều-nhiều, V4.17)**: click [×] → confirm dialog "Xóa {chiến dịch/kênh} [Giá trị] khỏi danh sách chặn của [SĐT]? Thao tác này gỡ [N] tổ hợp: [Campaign1 · Kênh1], [Campaign2 · Kênh2]..." [Hủy] / [Xóa] → xác nhận: gỡ toàn bộ N tổ hợp chứa giá trị đó, toast "Đã xóa N bản ghi ✓"; dòng cập nhật lại chip còn lại <br>**Dòng Toàn hệ thống chỉ Admin thấy nút [Xóa]**, QTV thấy dạng chỉ đọc |
| 8b | Nút [Xem chi tiết tổ hợp] | Button | – | – | (V4.17) **Chỉ hiển thị khi dòng là ma trận nhiều-nhiều thật** — đồng thời ≥ 2 campaign VÀ ≥ 2 kênh; đặt cạnh nút [Xóa] toàn dòng; click → mở popover liệt kê đầy đủ từng tổ hợp (Campaign · Kênh) của số đó, mỗi dòng có [×] riêng để xóa đúng 1 tổ hợp cụ thể — điều mà chip đơn lẻ ở cột 6/7 không làm được (chip luôn gộp mọi tổ hợp chứa cùng giá trị); click [×] trên 1 dòng tổ hợp → confirm dialog "Xóa [số] khỏi blacklist campaign [Y] kênh [Z]? Số này vẫn còn bị chặn ở [N-1] tổ hợp khác." [Hủy] / [Xóa] → xác nhận: gỡ đúng tổ hợp đó, popover cập nhật danh sách còn lại, toast "Đã xóa ✓"; nếu sau khi xóa dòng không còn là ma trận nhiều-nhiều (chỉ còn 1 campaign hoặc 1 kênh) → ẩn nút này, quay lại hiển thị chip thường; đóng popover bằng nút [Đóng] hoặc click ra ngoài |
| 9 | Nút [+ Thêm thủ công] | Button | – | – | Mở Modal 6B với tất cả trường trống; Campaign/Kênh cho phép multi-select (xem 6B STT 2, 3) |
| 10 | Nút [📥 Upload danh sách] | Button | – | – | Mở Modal 6C; Campaign/Kênh cho phép multi-select (xem 6C STT 1, 2) |
| 11 | Nút [+ Thêm vào Blacklist toàn hệ thống] | Button | – | – | **Chỉ Admin thấy** — mở Modal riêng có 2 tab con: "Thêm thủ công" (giống 6B nhưng không có trường Campaign/Kênh, xem UC-BL-04) và "Upload CSV" (giống 6C nhưng không có trường Campaign/Kênh, xem UC-BL-05); tab mặc định "Thêm thủ công" |
| 12 | Phân trang | Pagination | – | 20/trang | Phân trang theo **số dòng đã gộp** (1 dòng/số/phạm vi), "< 1 2 3 >" + dropdown [20/trang ▾]; options: 20 / 50 / 100; reset về trang 1 khi thay đổi filter hoặc tìm kiếm; **Trạng thái rỗng** (không có số nào — bao gồm cả sau khi filter): "Danh sách blacklist đang trống" (chiếm toàn bộ hàng bảng) |
...
| 2 | Campaign | Dropdown (multi-select) | Có | Trống | Chọn từ danh sách campaign có sẵn; hiển thị tên + mã campaign; hỗ trợ chọn nhiều campaign cùng lúc (chip hiển thị từng campaign đã chọn); **Thời điểm validate**: khi nhấn [Thêm] — nếu chưa chọn campaign nào → chặn thao tác, hiển thị lỗi đỏ ngay dưới dropdown "Vui lòng chọn ít nhất 1 chiến dịch" |
| 3 | Kênh | Dropdown (multi-select) | Có | Push | Options: Push / Zalo OA / SMS / USSD / Banner / Email — tất cả 6 kênh, không lọc theo campaign; hỗ trợ chọn nhiều kênh cùng lúc; mặc định chọn sẵn Push; **Validate**: nếu QTV bỏ chọn hết (không còn kênh nào) → chặn thao tác khi nhấn [Thêm], hiển thị lỗi đỏ ngay dưới dropdown "Vui lòng chọn ít nhất 1 kênh" |
| 4 | Xử lý số sai định dạng và tổ hợp trùng | – | – | – | Khi nhấn [Thêm]: số sai định dạng bị lọc bỏ (không thêm, không chặn toàn bộ thao tác); mỗi số hợp lệ tạo N × M bản ghi (N = số Campaign đã chọn, M = số Kênh đã chọn); tổ hợp (số, Campaign, Kênh) đã tồn tại trong blacklist cũng bị lọc bỏ (không tạo bản ghi trùng, không báo lỗi riêng cho từng tổ hợp) — chỉ các tổ hợp hợp lệ và chưa trùng mới được lưu; sau khi lưu, các bản ghi cùng số điện thoại hiển thị gộp chung 1 dòng ở màn danh sách (xem Screen 6A) |
| 5 | Nút [Hủy] | Button | – | – | Đóng modal, không lưu; không cần confirm khi hủy |
| 6 | Nút [Thêm] | Button | – | – | Disabled khi ô Số điện thoại chưa có số hợp lệ nào (theo định dạng); **Khi nhấn**: (1) Campaign — chưa chọn campaign nào → chặn, hiển thị lỗi "Vui lòng chọn ít nhất 1 chiến dịch" (xem STT 2), dừng xử lý; (2) Kênh — chưa chọn kênh nào → chặn, hiển thị lỗi "Vui lòng chọn ít nhất 1 kênh" (xem STT 3), dừng xử lý; (3) đã có ít nhất 1 Campaign và 1 Kênh — hệ thống lọc số sai định dạng + tổ hợp trùng, lưu các bản ghi còn lại; nút bị khoá trong lúc xử lý → **Khi có ít nhất 1 bản ghi được lưu mới**: đóng modal + thêm các dòng mới lên đầu bảng danh sách + toast "Đã thêm X bản ghi vào blacklist ✓" (X = số bản ghi thực sự lưu mới, tính theo tổ hợp); **Khi 0 bản ghi được lưu mới** (toàn bộ tổ hợp đều đã trùng): modal **không đóng**, hiển thị lỗi đỏ ngay trong modal "Toàn bộ tổ hợp đã nhập đều đã có trong blacklist — không có bản ghi nào được thêm", nút mở khóa lại; **Khi hệ thống lỗi**: toast "Thêm thất bại — vui lòng thử lại" + modal giữ nguyên + nút mở khóa lại |
...
*Phiên bản: V4.17 | Ngày: 15/09/2026 | Tác giả: Jun (BA)*
*Đợt cập nhật gần nhất: xóa 1 chip Kênh/Campaign trên dòng Blacklist ma trận nhiều-nhiều (≥2 campaign VÀ ≥2 kênh) gỡ TẤT CẢ tổ hợp chứa giá trị đó; bổ sung nút [Xem chi tiết tổ hợp] để xóa lẻ đúng 1 tổ hợp cụ thể; text confirm dialog phân biệt rõ theo từng case xóa.*
