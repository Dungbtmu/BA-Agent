...
**Gửi lại (Retry gửi cho cùng đối tượng) sau lần gửi đầu tiên:**
- Áp dụng ở mức **campaign** (không phải toàn hệ thống); Admin cấu hình tại System Settings mục Frequency Cap: bật/tắt "Cho phép gửi lại", nếu bật thì nhập **Số lần gửi lại tối đa** (số nguyên dương) và **Khoảng cách tối thiểu giữa các lần gửi lại** (đơn vị giờ)
- Chỉ áp dụng cho trường hợp lần gửi đầu **thất bại** (Failed) hoặc **bị chặn bởi Throttle** — không gửi lại cho message đã Delivered thành công
- Mỗi lần gửi lại vẫn phải tuân thủ toàn bộ điều kiện dừng pipeline khác (II.6.4) và các ngưỡng Frequency Cap ở trên — gửi lại không phải là ngoại lệ vượt qua Throttle
- Đạt số lần gửi lại tối đa mà vẫn thất bại → dừng hẳn, ghi log `RETRY_EXHAUSTED`, không gửi lại nữa cho lần trigger đó
...
| **Hoạt động** | 1. Hệ thống tải danh sách template, **nhóm theo Trigger áp dụng** (mỗi nhóm là 1 khối có thể thu gọn/mở rộng, tiêu đề nhóm = code + tên trigger; nhóm "Chưa gắn Trigger" luôn hiển thị cuối cùng); trong mỗi nhóm, template **sắp xếp theo số lần dùng nhiều nhất**; template gắn nhiều Trigger xuất hiện lặp lại ở từng nhóm tương ứng <br>1a. Hiển thị bảng mỗi nhóm: Tên Template, Kênh hỗ trợ, Số lần dùng, Hành động <br>2. QTV tùy chọn nhập từ khóa tìm kiếm theo tên template <br>2a. Hệ thống lọc realtime trong phạm vi tất cả nhóm; nhóm không có kết quả tự ẩn <br>3. QTV tùy chọn lọc theo Kênh hoặc Trạng thái (Active/Inactive) <br>3a. Hệ thống lọc bảng theo điều kiện đã chọn, áp dụng trong từng nhóm <br>4. QTV click số "X lần" ở cột Dùng <br>4a. Hệ thống hiển thị popover danh sách campaign đang dùng template đó <br>5. QTV click [Xóa] trên một template <br>5a. Hệ thống hiển thị confirm dialog "Xóa template [Tên]? Hành động này không thể hoàn tác." [Hủy] / [Xóa] <br>5b. Xác nhận → nếu template chưa được campaign nào tham chiếu (cột Dùng = 0): xóa cứng, toast "Đã xóa template ✓", biến mất khỏi danh sách <br>**[Exception — Xóa template đang được campaign dùng]**: nút [Xóa] disabled; hover → tooltip "Không thể xóa — template đang được N campaign sử dụng (kể cả Draft/Ended). Vui lòng gỡ khỏi các campaign đó trước, hoặc dùng [Tắt] để ẩn khỏi danh sách chọn mới" <br>**[Alternative]**: QTV click [+ Tạo Template] → navigate /templates/new |
| **Quy tắc nghiệp vụ** | - Cột "Dùng" hiển thị tổng số campaign có tham chiếu template này, kể cả Draft và Ended (không chỉ Active) — click → popover liệt kê tên campaign kèm trạng thái <br>- Template Inactive hiển thị grayed out trong danh sách; không xuất hiện trong dropdown khi soạn nội dung campaign <br>- Tìm kiếm áp dụng cho tên template <br>- **Nhóm theo Trigger**: dựa trên trường Trigger áp dụng khai báo tại UC-TPL-01/UC-TPL-03; chỉ là cách tổ chức hiển thị, không giới hạn phạm vi sử dụng template khi soạn campaign <br>- **Xóa template**: chỉ cho phép xóa cứng khi template chưa được bất kỳ campaign nào tham chiếu (Dùng = 0, tính cả Draft/Ended); template đã từng được dùng — dù campaign đó đã Ended — không thể xóa, chỉ có thể [Tắt] để ẩn khỏi lựa chọn mới, vì campaign cũ vẫn cần giữ nguyên nội dung đã copy tại thời điểm tạo (xem UC-TPL-03) |
...
| 7 | Cột ƯU TIÊN | Number (inline edit) | – | – | Hiển thị số độ ưu tiên hiện tại; **chỉ campaign Active**: click vào ô → chuyển sang input số inline, Enter/blur để xác nhận → confirm dialog → xác nhận thì campaign chuyển Pending (xem UC-CAM-01 bước 5, quy tắc nghiệp vụ); campaign Draft/Pending/Paused/Ended: hiển thị dạng text tĩnh, không click được |
...
| 2 | Ô tìm kiếm | Search input | Không | Trống | Placeholder: "Tìm tên template..."; tìm theo tên template; debounce 300ms, bắt đầu từ ký tự đầu tiên; áp dụng trong phạm vi tất cả nhóm Trigger; **Empty state**: không có kết quả → "Không có template nào phù hợp" (row span toàn bảng); **Loading state khi chuyển trang / apply filter**: spinner overlay trên bảng |
...
| 5 | Nhóm theo Trigger | Collapsible section | – | Mở rộng | Danh sách template được chia thành các khối theo Trigger áp dụng; tiêu đề khối = "[CODE] · [Tên trigger]" + số lượng template trong nhóm; nhóm "Chưa gắn Trigger" luôn ở cuối; mỗi khối có thể click để thu gọn/mở rộng; template gắn nhiều Trigger xuất hiện ở từng khối tương ứng |
| 6 | Cột Tên Template | Text | – | – | Tên hiển thị |
...
| 1.5 | Toggle "Cho phép gửi lại" | Toggle | – | Tắt | Bật → hiện 2 ô: "Số lần gửi lại tối đa" (số nguyên dương, placeholder "VD: 2") và "Khoảng cách tối thiểu giữa các lần (giờ)" (số nguyên dương, placeholder "VD: 4"); tắt → ẩn 2 ô, hệ thống không gửi lại khi thất bại |
| 1.6 | Ghi chú | Text | – | – | "Thay đổi áp dụng cho sự kiện tiếp theo — không hồi tố. Để trống một ngưỡng nghĩa là không giới hạn ở cấp đó." |
| 1.7 | Nút [Lưu cài đặt] | Button | – | – | Bị khoá khi có field lỗi; khi nhấn: nút bị khoá + đang xử lý → khi hệ thống phản hồi: form tải lại với giá trị vừa lưu + toast "Đã lưu cài đặt ✓"; nếu lỗi: toast "Lưu thất bại — vui lòng thử lại" + nút mở khoá lại |
| 1.8 | Trạng thái đang tải | Loading | – | – | Khi vào trang: form hiển thị trạng thái chờ cho đến khi lấy được giá trị hiện tại từ hệ thống |
| 1.9 | Trạng thái lỗi tải config | Error | – | – | Khi hệ thống không lấy được cấu hình: "⚠ Không tải được cấu hình — [↻ Thử lại]"; không hiển thị form trống |
| 1.10 | Validate từng ô | – | – | – | Nhập 0 hoặc số âm → "Giá trị không hợp lệ"; nhập > 9999 → "Giá trị tối đa là 9999"; để trống → hợp lệ (nghĩa là không giới hạn) |
| 1.11 | Validate tương quan | – | – | – | Khi cả Tuần (1.2) và Ngày (1.1) đều có giá trị và Tuần < Ngày → lỗi ngay dưới ô 1.2: "Giới hạn tuần phải ≥ giới hạn ngày"; khi cả Tháng (1.3) và Tuần (1.2) đều có giá trị và Tháng < Tuần → lỗi ngay dưới ô 1.3: "Giới hạn tháng phải ≥ giới hạn tuần"; Toggle gửi lại bật mà chưa nhập đủ 2 ô → "Vui lòng nhập đầy đủ số lần và khoảng cách gửi lại" |
...
*Phiên bản: V4.0 | Ngày: 21/08/2026 | Tác giả: Jun (BA)*
