...
| G4.U08 | Gia hạn gói liên tiếp – Vinh danh trung thành | Tôn vinh KH đạt mốc gia hạn liên tiếp, bao gồm gia hạn đúng hạn và gia hạn sớm theo rule cấu hình của CVM. Nâng hạng hoặc trao quyền lợi mới khi đạt milestone. | Offline — Push CSV | `consecutive_renewals ≥ x` với x do CVM cấu hình; chuỗi không bị gián đoạn dịch vụ | **Bước 1:** OCS ghi nhận giao dịch gia hạn gói. BSS nhận qua batch nightly, lưu lịch sử gia hạn. **Bước 2:** Sau mỗi lần gia hạn, BSS tính số chu kỳ gia hạn liên tiếp đủ điều kiện, phân loại `renewal_pattern` (`ON_TIME_ONLY`, `EARLY_ONLY`, `MIXED`). **Bước 3:** Khi đủ mốc cấu hình, BSS tạo file CSV `renewal_loyalty_YYYYMMDD.csv` push vào SFTP lúc 02:00–04:00. **Bước 4:** CVM đọc file 04:00–08:00, tính điểm trung thành, xác định milestone/hạng/ưu đãi. **Bước 5:** CVM gửi Banner + Email lúc 08:00–09:00. | Đã tặng thưởng cùng milestone trong chu kỳ gần đây; KH đã ở hạng cao nhất rồi; chuỗi gia hạn bị gián đoạn | `OCS (nightly) → BSS → Push CSV → CVM → Điểm trung thành / milestone → App + Email` | Banner App/Web + Email |
...
| U08 | Số chu kỳ gia hạn liên tiếp đủ điều kiện | 🟡 BSS + 🔴 OCS | `resource.msisdns` | `expiry_date` (lịch sử) | Bao gồm đúng hạn và sớm hạn theo rule CVM |
| U08 | Có gia hạn sớm trong chuỗi không | 🔴 OCS | — | `includes_early_renewal` | Bắt buộc sau khi gộp U_RENEWAL_STREAK vào U08 |
| U08 | Kiểu chuỗi gia hạn | 🟡 BSS + 🔴 OCS | — | `renewal_pattern` | `ON_TIME_ONLY` / `EARLY_ONLY` / `MIXED` |
| U08 | Ngày gia hạn lần cuối | 🟡 BSS | — | `last_renewal_date` | Ngày gia hạn lần cuối |
| U08 | Ngày hết hạn gói hiện tại | 🟡 BSS | `resource.msisdns` | `expiry_date` | Sau lần gia hạn gần nhất |
